...
from ..io.meshes import bounds_center_offset, centered_copy
...
def center_geometry_on_origin(geometry, bounds: Sequence[float]):
    """Return a copy of ``geometry`` centred on ``bounds``; the input is unchanged."""
    return centered_copy(geometry, bounds)


def center_result_if_requested(result, args):
    """Return a centred copy of a mesh result when the caller asked for it.

    The result itself is never modified. Results without vertices to move,
    such as a raster, are returned as they are: centring only means something
    for geometry.
    """
