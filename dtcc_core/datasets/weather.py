...
def _parse_latest_hour_csv(text: str):
    ...
    for i, line in enumerate(lines):
        ...
        if "period_to" not in meta:
            match = period_to_pattern.search(line)
            if match:
                meta["period_to"] = match.group(1).strip()
...
class WeatherDataset(DatasetDescriptor):
    ...
    source = [
        {
            "name": "SMHI Meteorological Observations download API",
    ...
    collection_period = (
        "Latest-hour snapshot; each parameter payload supplies the observation "
    ...
    processing_steps = [
        "Resolve requested meteorological parameters from SMHI IDs, numeric strings, or aliases",
        "For each parameter, fetch station-set/all latest-hour CSV from the SMHI metobs download API",
        "Parse parameter name, unit, timestamp, station coordinates, values, and quality codes from the CSV payload",
    ...
    presentation_headline = "Latest-Hour SMHI Weather Stations"
    presentation_summary = (
        "Point observations from SMHI meteorological stations, filtered to the "
        "requested bounds and carrying one field per weather parameter."
    )
    presentation_narrative = [
        {
            "heading": "What you are seeing",
            "body": (
                "Each point is an SMHI meteorological station with latest-hour "
                "measurements such as air temperature, wind, humidity, precipitation, "
                "or pressure."
            ),
        },
        {
            ...
            "body": (
                "Values are station observations, not gridded forecasts or "
                "interpolated weather fields. Quality codes are preserved as "
                "station attributes named q_<field>."
            ),
        },
        {
            "heading": "Limitations",
            "body": (
                "Station coverage is sparse and uneven. Empty results can be a "
                "valid no-station outcome for a small bbox, while partial results "
                "are marked when a requested parameter fetch fails."
            ),
        },
    ]
    key_points = [
        "Default parameters are IDs 1, 3, 4, 6, 7, and 9",
        "Field names can use stable DTCC names or Swedish SMHI parameter names",
        "Coordinates are filtered in WGS84 and returned in the requested CRS",
        "parameter_metadata records IDs, units, endpoint paths, and timestamps for fetched parameters",
    ]
    presentation_legend = {
        "title": "Weather station fields",
        "entries": [
            {"label": "air_temperature", "meaning": "SMHI parameter 1, celsius"},
    ...
    presentation_warnings = [
        ...
        "Station observations are not interpolated and should not be read as area-wide weather surfaces.",
    ...
    presentation_limitations = [
        "Station density and parameter availability vary by location.",
