...
from ..model import (
    Bounds, Grid, LineString, Mesh, MultiLineString, MultiSurface, Object, Point, PointCloud,
    Raster, RoadNetwork, Solid, Surface, Tree, VolumeGrid, VolumeMesh,
)
from .style import (apply_dtcc_style, get_theme, require_matplotlib,
                    resolve_colormap, style_colorbar)
...
def _rank(record):
    g = record.geometry
    dimension = (3 if isinstance(g, (Solid, MultiSurface, Surface, Mesh, VolumeMesh))
                 else 2 if isinstance(g, (Raster, Grid, VolumeGrid, Bounds))
                 else 1)
    try:
        lod = float(record.lod) if record.lod is not None else -1.
    except ValueError:
        lod = -1.
...
def _entries(root, lod, representation, field, check_crs):
    ...
    while stack:
        ...
        if not np.array_equal(obj.transform.affine, np.eye(4)):
            raise ValueError(f'Plotting Object transforms is undefined ({obj.id}); use explicit geometry coordinates')
        check_crs(obj.transform.srs)
        candidates = [(key, r) for key, r in obj.geometry.items()
                      if (representation is None or key == representation)
                      and (lod is None or r.lod == lod)
                      and (field is None or any(f.name == field for g in _shapes(r.geometry)
                                                for f in getattr(g, 'fields', [])))]
        if candidates:
            key, record = max(candidates, key=lambda item: _rank(item[1]))
            result.append((obj, key, record.geometry))
        elif not obj.geometry and lod is None and representation is None and field is None:
            if isinstance(obj, (Tree, RoadNetwork)):
                result.append((obj, None, obj))
        stack.extend(reversed([child for group in obj.children.values() for child in group]))
    return result
...
def _xyz(points):
    ...
    if not p.size: return np.empty((0, 3))
    p = np.atleast_2d(p)
    if p.shape[1] == 2:
...
def _grid_points(g, association, ids):
    ...
    if isinstance(g, VolumeGrid):
        ...
        z = b.zmin+(k+(.5 if cell else 0))*b.depth/max(g.depth, 1)
    else:
        j, i = np.unravel_index(ids, (ny, nx))
        z = np.full(len(ids), b.zmin)
    return np.column_stack([b.xmin+(i+(.5 if cell else 0))*b.width/max(g.width, 1),
                            b.ymin+(j+(.5 if cell else 0))*b.height/max(g.height, 1), z])
...
def _field_locations(g, f, limit, notes):
    ...
    if isinstance(g, (Grid, VolumeGrid)) and a in ('vertex', 'cell'):
        ...
        expected = 1
        if isinstance(g, Point): points = [[g.x, g.y, g.z]]
        elif isinstance(g, (Grid, VolumeGrid)):
            b = _domain(g)
            points = [[(b.xmin+b.xmax)/2, (b.ymin+b.ymax)/2, (b.zmin+b.zmax)/2]]
        else:
            arrays = [getattr(s, 'points', getattr(s, 'vertices', np.empty((0, 3)))) for s in _shapes(g)]
...
def _plot_model(root, *, ax, lod, representation, field, max_elements, theme, show):
    ...
    for name, value in [('lod', lod), ('representation', representation), ('field', field)]:
    ...
    def check_crs(srs):
        if not srs or srs in crs: return
        if crs:
            from pyproj import CRS
            for value in (crs[0], srs):
                if value not in parsed: parsed[value] = CRS.from_user_input(value)
            if parsed[crs[0]] != parsed[srs]:
                raise ValueError(f'Preview requires a common CRS; found {crs[0]!r} and {srs!r}')
        crs.append(srs)
    ...
    palette = {'Building': '#E6CC79', 'BuildingPart': '#E6CC79', 'Tree': '#78A55B',
               'Terrain': '#42665B', 'Landuse': '#60966B', 'WaterBody': '#63A8DF',
               'Road': '#929AA0', 'Railway': '#BEA6CB', 'PlantCover': '#78A55B'}
    ...
    def add(kind, coordinates, color):
        ...
        if not len(coordinates) or not np.all(np.isfinite(coordinates)): return
        remaining -= 1
        if kind == 'polygon':
    ...
    def draw(g, owner, rep_id, color, inherited=None):
        ...
        if transform is not None:
            ...
            if not np.array_equal(transform.affine, np.eye(4)):
                if not np.array_equal(matrix, np.eye(4)):
                    raise ValueError('Preview cannot compose nested nonidentity geometry transforms')
        ...
        if field is not None:
            for f in getattr(g, 'fields', []):
                ...
                if isinstance(g, (Solid, MultiSurface, MultiLineString)) and any(
                        not np.array_equal(s.transform.affine, np.eye(4)) for s in list(_shapes(g))[1:]):
                    raise ValueError('Aggregate field locations require shape children in the enclosing geometry frame')
                p, v = _field_locations(g, f, max(0, min(n, remaining)), notes)
                p = world(p)
                mask = np.isfinite(v) & np.all(np.isfinite(p), axis=1)
                if np.any(mask):
                    numeric_points.append(p[mask]); numeric_values.append(v[mask])
        ...
        if isinstance(g, (Solid, MultiSurface, MultiLineString)):
            if isinstance(g, Solid):
                ...
                if len(g.shells) > 1: notes.add('Solid exterior shells only')
            else:
            ...
            if not len(g.vertices): return
            if g.holes:
                notes.add('Holed surfaces shown as ring outlines')
                for ring in [g.vertices, *g.holes]: add('line', world(np.vstack([ring, ring[:1]])), color)
            else:
                add('polygon', world(g.vertices), color)
        elif isinstance(g, Mesh):
            for i in _indices(len(g.faces), n, notes): add('polygon', world(g.vertices[g.faces[i]]), color)
            ...
            for i in _indices(len(g.cells), max(1, n//6), notes):
                ...
                for a, b in ((0,1),(0,2),(0,3),(1,2),(1,3),(2,3)): add('line', v[[a,b]], color)
        elif isinstance(g, (Grid, VolumeGrid, Bounds)):
            b = g if isinstance(g, Bounds) else _domain(g)
            v = world([[xx, yy, zz] for zz in (b.zmin, b.zmax) for yy in (b.ymin, b.ymax) for xx in (b.xmin, b.xmax)])
            notes.add('Grids shown as domain outlines')
            for a,b in ((0,1),(0,2),(1,3),(2,3),(4,5),(4,6),(5,7),(6,7),(0,4),(1,5),(2,6),(3,7)):
            ...
            x, y = g.georef * (cols+.5, rows+.5)
            records = owner.attributes.get('elevation_rasters', []) if owner is not None else []
            record = next((r for r in records if r['geometry_id'] == rep_id), None)
            z = values if record else np.zeros(len(values))
            if len(values):
                numeric_points.append(np.column_stack([x,y,z])); numeric_values.append(values)
                numeric_metadata.append(('elevation' if record else 'raster', record['unit'] if record else '', 1))
            remaining -= len(values)
            if not record: notes.add('Raster samples shown on z=0; values are colours')
        elif isinstance(g, Point):
            add('point', world([[g.x,g.y,g.z]]), color)
        elif isinstance(g, PointCloud):
            for p in world(g.points[_indices(len(g.points), n, notes)]): add('point', [p], color)
        elif isinstance(g, Tree):
            if g.position.size: add('point', world(g.position.reshape(-1,3)), color)
        elif isinstance(g, RoadNetwork):
            for i in _indices(len(g.edges), n, notes): add('line', world(g.vertices[g.edges[i]]), color)
        elif isinstance(g, LineString):
            ids = _indices(max(0, len(g.vertices)-1), n, notes)
            for i in ids: add('line', world(g.vertices[[i,i+1]]), color)
        else:
            raise TypeError(f'No quick model preview for {type(g).__name__}')

    for owner, rep_id, g in entries:
        name = ((owner.semantic_type or type(owner).__name__).rsplit('#', 1)[-1]
                if owner is not None else type(g).__name__)
        color = palette.get(name, '#78C8BE')
        labels[name] = color
        draw(g, owner, rep_id, color)
    if numeric_metadata and len(set(numeric_metadata)) > 1:
        raise ValueError('A single colour scale requires matching field names, units and component counts')
    all_points = [*polygons, *lines, *numeric_points]
    if points: all_points.append(np.asarray(points))
    ...
    apply_dtcc_style(ax, theme=theme, axis='off', title=f'{type(root).__name__} preview')
    if polygons:
        # One collection lets Matplotlib depth-sort terrain and buildings together.
        ax.add_collection3d(Poly3DCollection(polygons, facecolors=polygon_colors,
                                            edgecolors=get_theme(theme)['grid'], linewidths=.25))
    if lines: ax.add_collection3d(Line3DCollection(lines, colors=line_colors, linewidths=.7))
    if points:
        p = np.asarray(points)
    ...
    if numeric_points:
        ...
        mask = np.isfinite(v) & np.all(np.isfinite(p), axis=1)
        artist = ax.scatter(*p[mask].T, c=v[mask], s=8, cmap=resolve_colormap(), depthshade=False)
        name, unit, dim = numeric_metadata[0]
        label = name + (' magnitude' if dim > 1 else '') + (f' [{unit}]' if unit else '')
        style_colorbar(ax.figure.colorbar(artist, ax=ax, shrink=.55, pad=.02, label=label), theme=theme)
    elif labels:
        from matplotlib.patches import Patch
        ax.legend(handles=[Patch(color=c, label=name) for name,c in sorted(labels.items())],
                  loc='upper left', fontsize=8, framealpha=.85)
    lo, hi = coordinates.min(axis=0), coordinates.max(axis=0)
    span = hi-lo
    padding = np.maximum(span*.03, max(float(span.max())*.01, .5))
    ax.set(xlim=(lo[0]-padding[0],hi[0]+padding[0]), ylim=(lo[1]-padding[1],hi[1]+padding[1]),
           zlim=(lo[2]-padding[2],hi[2]+padding[2]))
    ax.set_box_aspect(span+2*padding)
    ax.view_init(elev=35, azim=-65)
    if notes:
        ax.text2D(.01, .01, '\n'.join(sorted(notes)), transform=ax.transAxes,
