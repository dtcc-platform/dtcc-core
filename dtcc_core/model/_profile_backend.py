...
def issue(node, slot, rule, message):
    ...
    return {"path": f"objects[{node['id']!r}]{suffix}", "rule": rule, "message": message,
            "node_id": node['id'], "slots": parts}
...
def graph_checks(nodes, values, references):
    ...
    for node in nodes:
        if node["id"] in index:
            errors.append(issue(node, "id", "duplicate_id", "ID is not unique in this model"))
        index[node["id"]] = node
    if errors:
        return errors  # Ambiguous IDs must not resolve to an arbitrary occurrence.
    parents = {}
    for node, data in zip(nodes, values):
        for slot, (allowed, containment) in references.get(node["semantic_type"], {}).items():
            ...
            for target_id in targets:
                ...
                if target is None:
                    errors.append(issue(node, slot, "dangling_id", f"No object {target_id!r}"))
                elif target["semantic_type"] not in allowed:
                    errors.append(issue(
                        node, slot, "target_type",
    ...
    for start in parents:
        ...
        while current in parents and current not in done:
            if current in path:
                errors.append(issue(
                    index[current], "parent", "containment_cycle", "Containment contains a cycle",
                ))
                break
            path.add(current)
...
class LinkMLProfile:
    def __init__(self, path, *, closed=True, self_contained=False):
        ...
        if self_contained:
            ...
            if not isinstance(data, dict) or data.get('imports'):
                raise ValueError('Select a self-contained local LinkML schema without imports')
        ...
        self.record_classes = {name for name in self.classes
                               if self.view.get_identifier_slot(name) is None}
        self.uris = {name: self.view.get_uri(cls, expand=True)
        ...
        for name, cls in self.classes.items():
            ...
            self.slots[self.uris[name]] = {slot.name for slot in self.view.class_induced_slots(name)}
        ...
        for name in self.classes:
            for slot in self.view.class_induced_slots(name):
                annotation = getattr(slot.annotations, 'dtcc_representation_reference', None)
                if annotation is None:
                    continue
                target = self.classes.get(str(annotation.value))
                binding = target.annotations.get('dtcc_native_type') if target else None
                if (name not in self.record_classes or slot.range != 'string'
                        or slot.multivalued or slot.any_of or binding is None):
                    raise ValueError(
                ...
                record_references.setdefault(name, []).append((slot.name, str(binding.value)))
        self.representation_references = {}
        object_class = self.class_names.get(self.native_types.get('Object'))
        object_classes = (set(self.view.class_descendants(object_class, reflexive=True))
                          if object_class is not None else set())
        for name in self.classes:
            for slot in self.view.class_induced_slots(name):
                ranges = [option.range for option in slot.any_of] or [slot.range]
                candidates = {descendant for target in ranges if target in self.classes
                              for descendant in self.view.class_descendants(target, reflexive=True)}
                if not candidates.intersection(record_references):
                    continue
                if (name not in object_classes or not (slot.inlined or slot.inlined_as_list)
                        or slot.any_of or slot.range not in record_references):
                    raise ValueError(
        ...
        for kind in self.classes:
            ...
            for slot in self.view.class_induced_slots(kind):
                ranges = [option.range for option in slot.any_of] or [slot.range]
                class_ranges = {descendant for target in ranges if target in self.classes
                                for descendant in self.view.class_descendants(target, reflexive=True)}
                if class_ranges and (slot.inlined or slot.inlined_as_list):
                    if self_contained and any(target not in self.record_classes for target in class_ranges):
                        raise ValueError(f'{kind}.{slot.name}: inlined records must be identifier-free')
                ...
                if self_contained and any(target in self.classes for target in ranges):
                    if not all(target in self.classes for target in ranges):
                        raise ValueError(f'{kind}.{slot.name}: expected non-inlined class ID references')
                    if kind in self.record_classes or any(target in self.record_classes for target in class_ranges):
                        raise ValueError(f'{kind}.{slot.name}: records require explicit inlining and cannot contain ID references')
                if not all(target in self.classes for target in ranges):
                    continue
                allowed = {
                    self.uris[descendant] for target in ranges
                    for descendant in self.view.class_descendants(target, reflexive=True)
                }
                annotation = getattr(slot.annotations, "containment", None)
                containment = annotation is not None and annotation.value is True
                if self_contained and containment and (slot.name != 'parent' or slot.multivalued):
                    raise ValueError(f'{kind}.{slot.name}: containment must be the scalar parent slot')
    ...
    def validate(self, nodes, values):
        ...
        for node, data in zip(nodes, values):
            ...
            if kind not in self.classes or self.classes[kind].abstract or kind in self.record_classes:
                errors.append(issue(
                    node, "semantic_type", "unknown_type",
            ...
            for slot in sorted(node['relations'].keys() - self.references[node['semantic_type']].keys()):
                errors.append(issue(node, slot, 'unknown_relation', 'Relationship is not declared by this profile'))
            missing_paths = set()
                ...
                if source.validator == 'required':
                    for missing in source.validator_value:
                        ...
                        if missing not in source.instance and path not in missing_paths:
                            ...
                            errors.append(issue(node, path, 'required', f'Missing required value {missing!r}'))
                else:
                    errors.append(issue(node, slot, str(source.validator), source.message))
