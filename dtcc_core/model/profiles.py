...
def _project(
    ...
    *,
    native_types=None,
    slots=None,
    representation_references=None,
    ...
    def semantic_type(value, explicit=None):
        ...
        try:
            ...
            raise NotImplementedError(
                f"No schema binding for native {type(value).__name__}"
            ) from exc
    ...
    def append(
        ...
        if native_types is not None:
            ...
            attributes = {
                ...
                if key in slots[semantic_type]
                and key not in {"id", "semantic_type", "parent", *reserved}
            }
        for key in sorted(
            relations.keys() & ({"id", "semantic_type"} | intrinsic.keys())
        ):
            raise ValueError(
                f"{base}.relations[{key!r}]: conflicts with a projected native field"
            )
        reserved = (
            {"id", "semantic_type", "parent", *reserved}
            | intrinsic.keys()
            | relations.keys()
        )
        for key in sorted(attributes.keys() & reserved):
            raise ValueError(
                f"{base}.attributes[{key!r}]: conflicts with a projected native field"
            )
        records.append(
            {
                "id": id,
    ...
    def project_geometry(value, base, owner_id, owner_path, region_key):
        ...
        while pending:
            ...
            for index, region in enumerate(getattr(geometry, "regions", ())):
                ...
                extra_refs = (references or {}).get(kind, {}).keys() - {
                    "parent",
                    "host",
                }
                if extra_refs:
                    raise NotImplementedError(
                        f"{region_base}: region references support only owner parent and host"
                    )
                ...
                fields = {
                    "id": region_base,
                    "semantic_type": f"{region_base}.semantic_type",
                    "parent": parent_path,
                    "host": f"{region_base}.parent",
                }
                append(
                    json.dumps([*key, index]),
                    kind,
                    region.attributes,
                    relations,
                    region_base,
            ...
            if native_types is not None:
                child_key = (
                    "linestrings" if hasattr(geometry, "linestrings") else "surfaces"
                )
    ...
    while stack:
        ...
        relations = {
            name: [object_id(id) for id in ids] for name, ids in obj.relations.items()
        }
        fields = {name: f"{base}.relations[{name!r}]" for name in relations}
        fields.update(
            id=f"{base}.id",
            semantic_type=f"{base}.semantic_type",
            parent=containment_path or base,
        )
        for slot in (references or {}).get(kind, {}):
            ...
            if slot in obj.attributes and (native_types is None or slot != "parent"):
                raise ValueError(
                    f"{base}.attributes[{slot!r}]: class references belong in Object.relations"
                )
        ...
        append(
            object_id(obj.id), kind, obj.attributes, relations, base, fields, intrinsic
        )
        for slot, multiple, field, native in (representation_references or {}).get(
            kind, ()
        ):
        ...
        for key, record in obj.geometry.items():
            project_geometry(
                record.geometry,
                f"{base}.geometry[{key!r}].geometry",
                object_id(obj.id),
                base,
                ["region", obj.id, key],
            )
        children = [
            (child, obj, f"{base}.children[{cls.__name__}][{index}]")
            for cls, group in obj.children.items()
...
class SemanticProfile:
    ...
    def _validate_admitted(self, model):
        ...
        native_types = (
            self._backend.native_types if self._backend.open_semantics else None
        )
        representation_issues = []
        records, locations = _project(
            model,
            self._backend.references,
        ...
        values = [
            {"id": node["id"], **node["attributes"], **node["relations"]}
            for node in records
        ]
