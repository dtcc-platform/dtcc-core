...
def _profile(schema_id, version):
    ...
    if (schema_id, version) != (SCHEMA_ID, DEFAULT_VERSION):
        raise ValueError(
            f"Unsupported semantic schema {schema_id!r} version {version!r}"
        )
    ...
    if (profile.profile_id, profile.profile_version) != (schema_id, version):
        raise ValueError(
            "Bundled schema declaration does not match its selected identity/version"
        )
    return profile
...
def validate_admitted(model, schema_id, version):
    ...
    if not report.valid:
        details = "\n".join(
            f"{issue.path}: {issue.message}" for issue in report.issues[:10]
        )
        if len(report.issues) > 10:
            details += f"\n... and {len(report.issues) - 10} more issues"
        raise ValueError(
            f"Schema validation failed ({schema_id}, {version}):\n{details}"
        )
