...
_MESHER_READY_ACUTE_TIP_NEARBY_SEGMENT_FACTOR = 1.25
...
def _normalize_mesher_ready_polygon(
    ...
    *,
    ...
    local_diagnostics.setdefault("geos_exception_messages", [])

    def regularize_single_polygon(base_polygon: Polygon) -> Polygon:
        ...
        def consider_candidate(candidate: _RepairCandidate | None) -> None:
            nonlocal best_polygon, best_score, best_operator
            if candidate is None:
                return
            canonical_parts: list[Polygon] = []
            for part in _canonicalize(candidate.polygon, grid, local_diagnostics):
                ...
                canonical_parts.extend(
                    _canonicalize(without_small_holes, grid, local_diagnostics)
                )
            filtered_parts = [
                orient(part, sign=1.0)
                for part in canonical_parts
                if part.area + 1.0e-12 >= 0.0
            ]
            ...
            if not _signature_improves(
                ...
                return
            candidate_metrics = _difference_area_metrics(base_polygon, accepted)
            if not _difference_metrics_have_small_fidelity_drift(
                candidate_metrics,
                edit_zone_area=float(base_polygon.area),
                target_scale=target_scale,
                grid=grid,
            ):
                return
            score = (
                ...
                float(candidate_signature.acute_tip_count),
                candidate_signature.acute_tip_span,
                float(candidate_signature.short_edge_count),
                -(candidate_signature.min_edge_length or 0.0),
                float(candidate_signature.vertex_count),
                float(candidate_metrics.get("symmetric_difference_area", 0.0)),
                abs(float(candidate_metrics.get("union_area_delta", 0.0))),
            )
            if best_score is None or score < best_score:
                best_polygon = accepted
                best_score = score
                best_operator = candidate.operator

        if reference_signature.clearance_deficit > max(grid, 1.0e-9):
            consider_candidate(
                _try_polygon_self_clearance_connector_fill(
                    base_polygon,
                    min_clearance=target_scale,
                    grid=grid,
                    diagnostics=local_diagnostics,
                )
            )
        if reference_signature.acute_tip_count > 0:
            consider_candidate(
                _try_polygon_acute_tip_connector_fill(
                    base_polygon,
                    min_clearance=target_scale,
                    grid=grid,
                    diagnostics=local_diagnostics,
        ...
        if diagnostics is not None:
            ...
            if best_operator is not None:
                operator_applied = diagnostics.setdefault(
                    "mesher_ready_polygon_operator_applied",
                    {},
                )
                operator_applied[best_operator] = (
                    int(operator_applied.get(best_operator, 0)) + 1
                )

        return best_polygon

    if not polygon.interiors or not _polygon_has_ring_boundary_contacts(polygon):
        return [regularize_single_polygon(polygon)]
    ...
    for _attempt in range(8):
        ...
        if candidate_polygons and all(
            ...
            return [
                regularize_single_polygon(candidate_polygon)
...
def _meshing_hostile_acute_tip_metrics(
    ...
    *,
    ...
    if target_scale <= 0:
        return 0, 0.0
    nearby_segment_limit = max(
        _MESHER_READY_ACUTE_TIP_NEARBY_SEGMENT_FACTOR * target_scale,
        1.0e-9,
    )
    acute_tip_count = 0
    acute_tip_span = 0.0
    for _candidate in _iter_polygon_acute_tip_candidates(
        ...
        vertex_xy = _candidate[5]
        projected_xy = _candidate[6]
        if (
            float(np.linalg.norm(vertex_xy - projected_xy))
            > nearby_segment_limit + 1.0e-12
        ):
            continue
        acute_tip_count += 1
        acute_tip_span += float(_candidate[4])
    return acute_tip_count, acute_tip_span
...
def _try_hole_cluster_self_clearance_merge(
    polygon: Polygon,
    *,
    start_ring_index: int,
    end_ring_index: int,
    ...
    try:
        ...
        return None
    if any(hole.is_empty for hole in holes):
        return None
    ...
    close_tolerance = max(grid, min_clearance * 1.0e-6, 1.0e-9)
    adjacency: dict[int, set[int]] = {index: set() for index in range(len(holes))}
    close_distances: list[float] = []
    for index, hole in enumerate(holes):
        for other_index in range(index + 1, len(holes)):
            other = holes[other_index]
            try:
                distance = float(hole.distance(other))
            except GEOSException as exc:
                _record_geos_exception(diagnostics, "hole_cluster_clearance_merge", exc)
                continue
            if distance <= min_clearance + close_tolerance:
                adjacency[index].add(other_index)
                adjacency[other_index].add(index)
                close_distances.append(distance)

    component: set[int] = set()
    pending = [start_ring_index, end_ring_index]
    while pending:
        index = pending.pop()
        if index in component:
            continue
        component.add(index)
        pending.extend(sorted(adjacency[index] - component))
    if len(component) <= 2:
        return None
    ...
    other_holes = [
        list(ring.coords)
        for ring_index, ring in enumerate(polygon.interiors)
        if ring_index not in component
    ]
    ...
    def consider_merged_hole(merged_hole: BaseGeometry) -> None:
        ...
        if not candidate_is_acceptable(candidate_signature):
            return
        ...
        if best is None or score < best[0]:
            best = (score, candidate_polygon, edit_zone)

    current_clearance = min(close_distances) if close_distances else min_clearance
    for distance in _iter_hole_pair_merge_distances(
        min_clearance=min_clearance,
        current_clearance=current_clearance,
        grid=grid,
    ...
    try:
        consider_merged_hole(unary_union(component_holes).convex_hull)
    except GEOSException as exc:
        _record_geos_exception(diagnostics, "hole_cluster_clearance_merge", exc)
    ...
    return _RepairCandidate(
        polygon=best[1],
        edit_zone=best[2],
        operator="hole_cluster_clearance_merge",
        area_balance_budget_override=area_budget_override,
    )


def _try_hole_pair_clearance_fill_smaller_hole(
    polygon: Polygon,
    *,
    start_ring_index: int,
    end_ring_index: int,
    ...
    for drop_index in sorted(
        ...
        key=lambda index: Polygon(polygon.interiors[index]).area,
    ):
        holes = [
            list(ring.coords)
            for ring_index, ring in enumerate(polygon.interiors)
            if ring_index != drop_index
        ]
        candidate_polygon = _normalize_single_polygon_candidate(
        ...
        score = (
            ...
            difference_metrics["symmetric_difference_area"],
            abs(difference_metrics["union_area_delta"]),
            Polygon(polygon.interiors[drop_index]).area,
            candidate_signature.vertex_count,
        )
        if best is None or score < best[0]:
            best = (score, candidate_polygon, edit_zone)
    ...
    return _RepairCandidate(
        polygon=best[1],
        edit_zone=best[2],
        operator="hole_pair_clearance_fill",
        area_balance_budget_override=area_budget_override,
    )
...
def _try_same_hole_self_clearance_connector_fill(
    polygon: Polygon,
    *,
    clearance_coords: np.ndarray,
    hole_ring_index: int,
    ...
    hole_coords = np.asarray(
        polygon.interiors[hole_ring_index].coords[:-1],
        dtype=float,
    )
    ...
    def consider_displacement(
        ...
        for index, delta in displacement.items():
            updated_coords[index] = updated_coords[index] + delta
        holes: list[Sequence[Sequence[float]]] = []
        updated_ring = [
            (float(x), float(y))
            for x, y in np.vstack([updated_coords, updated_coords[0]])
        ]
        for candidate_hole_index, ring in enumerate(polygon.interiors):
            if candidate_hole_index == hole_ring_index:
                holes.append(updated_ring)
    ...
    contact_options: list[list[tuple[dict[int, np.ndarray], BaseGeometry]]] = []
    for contact_xy in (clearance_coords[0], clearance_coords[-1]):
        point = Point(float(contact_xy[0]), float(contact_xy[1]))
        point_xy = np.asarray(contact_xy, dtype=float)
        options: list[tuple[dict[int, np.ndarray], BaseGeometry]] = []

        vertex_index, vertex_distance = _nearest_vertex_index_and_distance(
            hole_coords,
            point_xy,
        )
        hole_direction = _polygon_inward_direction_at_contact(
        ...
        if hole_direction is not None and vertex_distance <= tolerance:
            options.append(
                (
                    {vertex_index: -hole_direction},
                    LineString(
                        [
                            tuple(hole_coords[vertex_index]),
                            tuple(point_xy),
                        ]
                    ).buffer(
                        max(grid, min_clearance * 0.25),
                        quad_segs=1,
                        join_style=BufferJoinStyle.mitre,
                        mitre_limit=1000.0,
                    ),
        ...
        if segment is not None:
            segment_index, segment_start, segment_end, projected_xy = segment
            if float(np.hypot(*(projected_xy - point_xy))) <= tolerance:
                segment_direction = _boundary_edge_inward_direction(
                    hole_coords,
                    projected_xy,
                    tolerance=max(tolerance, 1.0e-6),
                )
        ...
        contact_options.append(options)

    if not any(contact_options):
        return None

    contraction_distances = sorted(
        {
            max(4.0 * grid, min_clearance * factor, 1.0e-9)
            for factor in (0.25, 0.375, 0.5, 0.625, 0.75, 1.0)
        }
    )
    for distance in contraction_distances:
        scaled_options: list[list[tuple[dict[int, np.ndarray], BaseGeometry]]] = []
        for options in contact_options:
            scaled: list[tuple[dict[int, np.ndarray], BaseGeometry]] = []
            ...
            scaled_options.append(scaled)

        for options in scaled_options:
            for displacement, edit_zone in options:
                consider_displacement(displacement, edit_zone)

        if len(scaled_options) >= 2:
            for first_displacement, first_zone in scaled_options[0]:
                for second_displacement, second_zone in scaled_options[1]:
                    combined_displacement: dict[int, np.ndarray] = {}
                    for displacement in (first_displacement, second_displacement):
                        for index, delta in displacement.items():
                            existing = combined_displacement.get(index)
                            if existing is None:
                                combined_displacement[index] = delta.copy()
                            else:
                                combined_displacement[index] = existing + delta
                    combined_zone = unary_union([first_zone, second_zone])
                    consider_displacement(combined_displacement, combined_zone)

    if best is None:
        return None

    area_budget_override = max(float(best[2].area), 16.0 * grid * grid, 1.0e-9)
    return _RepairCandidate(
        polygon=best[1],
        edit_zone=best[2],
        operator="self_clearance_connector_fill",
...
def _try_polygon_self_clearance_connector_fill(
    ...
    *,
    ...
    if start_kind == end_kind and start_ring_index == end_ring_index:
        if start_kind == "hole" and start_ring_index is not None:
            same_hole_candidate = _try_same_hole_self_clearance_connector_fill(
                polygon,
                clearance_coords=clearance_coords,
                hole_ring_index=start_ring_index,
                min_clearance=min_clearance,
                grid=grid,
                diagnostics=diagnostics,
                require_signature_improvement=require_signature_improvement,
            )
            if same_hole_candidate is not None:
                return same_hole_candidate
    ...
    if (
        ...
        hole_cluster_candidate = _try_hole_cluster_self_clearance_merge(
            polygon,
            start_ring_index=start_ring_index,
            end_ring_index=end_ring_index,
            min_clearance=min_clearance,
        ...
        if hole_cluster_candidate is not None:
            return hole_cluster_candidate
        hole_fill_candidate = _try_hole_pair_clearance_fill_smaller_hole(
            polygon,
            start_ring_index=start_ring_index,
            end_ring_index=end_ring_index,
            min_clearance=min_clearance,
        ...
        if hole_fill_candidate is not None:
            return hole_fill_candidate
    if {start_kind, end_kind} == {"exterior", "hole"}:
        ...
        if hole_ring_index is not None:
            ...
            if exterior_hole_candidate is not None:
                return exterior_hole_candidate
...
def _regularize_coverage_for_meshing(
    ...
    *,
    ...
    def _repair_residual_self_clearance_for_meshing(
        ...
        split_diagnostics["collect_stage_metrics"] = False
        split_diagnostics["enable_logging"] = False
        split_polygons, split_sources = _split_low_clearance_neck_polygons_for_meshing(
            input_polygons,
            input_sources,
            min_clearance=min_segment_length,
            grid=grid,
        ...
        diagnostics["geos_exception_count"] += split_diagnostics["geos_exception_count"]
        diagnostics["geos_exception_messages"].extend(
            split_diagnostics["geos_exception_messages"]
        )
        split_applied_count = int(split_diagnostics.get("neck_split_applied_count", 0))
        if split_applied_count > 0:
            consider_variant(
                split_polygons,
                split_sources,
                operator_attempts={"meshing_contract_neck_split": split_applied_count},
                operator_applied={"meshing_contract_neck_split": split_applied_count},
            )
...
def _split_low_clearance_neck_polygons_for_meshing(
    polygons: list[Polygon],
    source_map: list[list[int]],
    *,
    min_clearance: float,
    ...
    diagnostics["neck_split_candidate_count"] = 0
    diagnostics["neck_split_applied_count"] = 0
    diagnostics["neck_split_failed_count"] = 0
    if min_clearance <= 0.0 or not polygons:
        return polygons, source_map

    output_polygons: list[Polygon] = []
    output_sources: list[list[int]] = []
    overlap_tolerance = max(grid * grid, 1.0e-9)
    ...
    for polygon, indices in zip(polygons, source_map):
        ...
        if (
            reference_signature.clearance is None
            or reference_signature.clearance + max(grid, 1.0e-9) >= min_clearance
        ):
            output_polygons.append(polygon)
            output_sources.append(list(indices))
            continue

        try:
            clearance_line = shapely.minimum_clearance_line(polygon)
        except GEOSException as exc:
            _record_geos_exception(diagnostics, "neck_split_clearance_line", exc)
            output_polygons.append(polygon)
            output_sources.append(list(indices))
            continue
        if clearance_line is None or clearance_line.is_empty:
        ...
        start_kind, start_ring_index, _ = _nearest_ring_classification(
            polygon,
            Point(float(clearance_coords[0][0]), float(clearance_coords[0][1])),
        )
        end_kind, end_ring_index, _ = _nearest_ring_classification(
            polygon,
            Point(float(clearance_coords[-1][0]), float(clearance_coords[-1][1])),
        )
        if (
            start_kind != "exterior"
            or end_kind != "exterior"
            or start_ring_index is not None
            or end_ring_index is not None
        ):
            output_polygons.append(polygon)
            output_sources.append(list(indices))
            continue

        diagnostics["neck_split_candidate_count"] += 1
        reference_coverage_signature = _coverage_defect_signature(
            [polygon],
            target_scale=min_clearance,
        )
        best: tuple[
        ...
        for half_width in (
            ...
            try:
                ...
                raw_parts = _canonicalize(
                    polygon.difference(cutter),
                    grid,
                    diagnostics,
                )
            except GEOSException as exc:
                _record_geos_exception(diagnostics, "neck_split_cut", exc)
                continue
            ...
            candidate_parts = [
                ...
                if part.area + 1.0e-12 >= min_area
            ]
            if len(candidate_parts) <= 1:
                continue

            candidate_sources = [list(indices) for _ in candidate_parts]
            local_diagnostics = _empty_diagnostics(len(candidate_parts))
            local_diagnostics["collect_stage_metrics"] = False
            local_diagnostics["enable_logging"] = False
            candidate_parts, candidate_sources = _simplify_polygons_for_meshing(
                candidate_parts,
                candidate_sources,
                min_segment_length=min_clearance,
                grid=grid,
            ...
            candidate_parts, candidate_sources = _regularize_low_clearance_polygons(
                candidate_parts,
                candidate_sources,
                min_clearance=min_clearance,
                grid=grid,
            ...
            if len(candidate_parts) <= 1:
                continue
            if _coverage_overlap_area(candidate_parts) > overlap_tolerance:
                continue
            ...
            if not _coverage_signature_improves(
                reference_coverage_signature,
                candidate_signature,
                grid=grid,
                target_scale=min_clearance,
            ) and not _coverage_signature_satisfies_scale_contract(
                candidate_signature,
                target_scale=min_clearance,
                grid=grid,
            ):
        ...
        if best is None:
            diagnostics["neck_split_failed_count"] += 1
            output_polygons.append(polygon)
            output_sources.append(list(indices))
            continue

        diagnostics["neck_split_applied_count"] += 1
        output_polygons.extend(best[1])
        output_sources.extend(best[2])

    if diagnostics["neck_split_applied_count"] == 0:
        return polygons, source_map
    return _stable_sort(output_polygons, output_sources)
