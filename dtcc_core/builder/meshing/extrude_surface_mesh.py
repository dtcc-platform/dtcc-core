...
def extrude_surface_to_solid(surface_mesh: Mesh, extrusion_depth: float = None, 
                           base_z: float = None) -> Mesh:
    """
    ...
    if extrusion_depth is None:
        extrusion_depth = (z_max - z_min) * 0.1  # 10% of height as default
    ...
    for face in surface_mesh.faces:
        solid_faces.append([vertex_map[face[0]], vertex_map[face[1]], vertex_map[face[2]]])
    ...
    for edge in boundary_edges:
        for vertex_idx in edge:
            if vertex_idx not in boundary_vertex_map:
                ...
                extruded_vertex = np.array([original_vertex[0], original_vertex[1], base_z])
    ...
    # Create bottom cap
    additional_bottom_vertices, bottom_faces = _create_bottom_cap(surface_mesh, boundary_vertex_map, base_z)
...
def _create_bottom_cap(surface_mesh: Mesh, boundary_vertex_map: Dict[int, int], 
                      base_z: float) -> Tuple[List[np.ndarray], List[List[int]]]:
    """
    ...
    for face in surface_mesh.faces:
        ...
        bottom_faces.append([bottom_face_indices[0], bottom_face_indices[2], bottom_face_indices[1]])
    
    return additional_vertices, bottom_faces


def create_printable_surface_mesh(surface_mesh: Mesh, 
                                extrusion_depth: float = None,
                                base_z: float = None,
                                minimum_thickness: float = 0.001) -> Mesh:
    """
    ...
    """
    # Basic extrusion
    solid_mesh = extrude_surface_to_solid(surface_mesh, extrusion_depth, base_z)
    
    # TODO: Add validation and fixes for 3D printing:
