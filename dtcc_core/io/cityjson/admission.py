...
from ...model import Building, BuildingPart, City, MultiSurface, Solid, Point, Object, Landuse, Terrain, Mesh, MultiLineString
...
FEATURE_TYPES = {'Building': Building, 'BuildingPart': BuildingPart, 'LandUse': Landuse,
                 'SolitaryVegetationObject': Object, 'CityFurniture': Object, 'TINRelief': Terrain,
                 **{name: Object for name in ('Road', 'Railway', 'Waterway', 'TransportSquare',
                                               'WaterBody', 'PlantCover')}}
FEATURE_GEOMETRIES = {
    'Building': (MultiSurface, Solid), 'BuildingPart': (MultiSurface, Solid),
    'LandUse': (MultiSurface,), 'SolitaryVegetationObject': (MultiSurface, Solid, Point),
...
def feature_type(value):
    ...
    for name, cls in FEATURE_TYPES.items():
        if type(value) is cls and (value.semantic_type == SEMANTIC_NAMESPACE + name
                                  or (cls not in (Object, Terrain) and value.semantic_type is None)):
            return name
    raise NotImplementedError('Object type/semantic URI needs an explicit CityJSON mapping')
...
def load_city(cj, *, extent_policy='validate'):
    ...
    for key in ('scale', 'translate'):
        ...
        if not isinstance(vector, list) or len(vector) != 3 or any(type(x) not in (int, float) for x in vector):
    ...
    for vector in (scale, translate):
        if vector.shape != (3,) or vector.dtype.kind not in 'ifu' or not np.isfinite(vector).all():
            raise ValueError('CityJSON scale/translate must have three finite numbers')
    if (scale <= 0).any():
    ...
    for vertex in source:
        if not isinstance(vertex, list) or len(vertex) != 3 or any(type(x) is not int for x in vertex):
    ...
    if extent is not None and (not isinstance(extent, list) or len(extent) != 6
    ...
    for id, obj in objects.items():
        _keys(obj, {'type', 'attributes', 'geometry', 'children', 'parents', 'geographicalExtent'}, id)
        ...
        value = FEATURE_TYPES[obj['type']](id=id, attributes=read_attributes(obj.get('attributes', {}), obj['type']),
        ...
        for index, geometry in enumerate(geometries):
            _keys(geometry, {'type', 'lod', 'boundaries', 'semantics'}, f'{id} geometry')
            ...
            native_geometry = (build_tin(geometry, vertices) if obj['type'] == 'TINRelief'
                               else build_geometry(geometry, vertices))
            if type(native_geometry) not in FEATURE_GEOMETRIES[obj['type']]:
                raise NotImplementedError(f'{id}: unsupported geometry for {obj["type"]}')
            validate_region_owner(native_geometry.regions, obj['type'])
            native_geometry.transform.srs = city.transform.srs
            role = (COMPOSITE_SURFACE_ROLE if type(native_geometry) is MultiSurface
                    and geometry['type'] == 'CompositeSurface' else None)
            value.add_geometry(native_geometry, id=f'cityjson-{index}', lod=lod, role=role)
        ...
        if children and obj['type'] not in ('Building', 'BuildingPart'):
            raise NotImplementedError('Strict containment currently supports building parts only')
        if obj['type'] != 'BuildingPart':
            if 'parents' in obj:
                raise ValueError('Top-level CityJSON feature cannot have parents')
        else:
            parent = obj.get('parents')
            if not isinstance(parent, list) or len(parent) != 1 or not isinstance(parent[0], str):
    ...
    discrepancies = []
    for source_id, extent, value in [(None, metadata.get('geographicalExtent'), city)] + [
            (id, objects[id].get('geographicalExtent'), value) for id, value in native.items()]:
        if _check_extent(extent, value, scale):
            if extent_policy == 'validate':
                raise ValueError("CityJSON extent does not match represented geometry; use extent_policy='recompute' to retain the discrepancy in Dataset Context")
            discrepancies.append({'object_id': source_id, 'source_extent': list(extent)})
    if discrepancies:
        ...
        city.dataset_context = DatasetContext(
            ...
            metadata=DatasetMetadata(crs=[city.transform.srs] if city.transform.srs else []),
            provenance=DatasetProvenance(processing_steps=[{
                'operation': 'recompute source extent summaries from unchanged geometry',
                'discrepancies': discrepancies,
            }]),
            presentation=DatasetPresentation(),
            request=DatasetRequest(dataset_name='cityjson-import', parameters={'extent_policy': extent_policy}),
            health={'status': 'warning', 'extent_discrepancy_count': len(discrepancies)},
            warnings=[f'Recomputed {len(discrepancies)} inconsistent CityJSON extent summaries; source summaries retained in provenance.'],
        )
    return city
...
def _check_extent(extent, value, scale):
    ...
    actual = [bounds.xmin, bounds.ymin, bounds.zmin, bounds.xmax, bounds.ymax, bounds.zmax]
...
def validate_export(city):
    ...
    while stack:
        ...
        if value is city and value.semantic_type not in (None, SEMANTIC_NAMESPACE + 'City'):
            raise NotImplementedError('City semantic URI needs an explicit CityJSON extension mapping')
        if type(value) is Landuse and value.landuses:
            raise NotImplementedError('Native Landuse codes need an explicit CityJSON mapping')
        if value.relations or value.profile_id is not None:
            raise NotImplementedError('CityJSON export cannot preserve named relations/profile identity without an extension')
        if value is city and (value.attributes or value.geometry):
            raise NotImplementedError('City root attributes/geometry need a CityJSON mapping')
        ...
        if not np.array_equal(value.transform.affine, np.eye(4)):
            raise NotImplementedError('CityJSON export requires geometry in the global coordinate frame')
        for group in value.children.values():
            for child in group:
                child_name = feature_type(child)
                if ((value is city and child_name == 'BuildingPart') or
                    (value is not city and (name not in ('Building', 'BuildingPart') or child_name != 'BuildingPart'))):
                    raise NotImplementedError('Strict CityJSON containment supports top-level features and building parts')
                stack.append(child)
        for index, (key, record) in enumerate(value.geometry.items()):
            if (key != f'cityjson-{index}' or
                    (record.role is not None and not
                     (record.role == COMPOSITE_SURFACE_ROLE and type(record.geometry) is MultiSurface))):
                raise NotImplementedError('CityJSON representation IDs/roles require an extension mapping')
            ...
            if type(geometry) not in FEATURE_GEOMETRIES[name]:
                raise NotImplementedError(f'Unsupported strict CityJSON geometry for {name}')
            ...
            if type(geometry) is Mesh:
                ...
                if geometry.markers.size or geometry.normals.size:
                    raise NotImplementedError('Mesh markers/normals require an explicit CityJSON mapping')
                if len(np.unique(geometry.faces)) != len(geometry.vertices):
                    raise NotImplementedError('Unused mesh vertices have no strict TINRelief mapping')
            lines = geometry.linestrings if type(geometry) is MultiLineString else []
            if type(geometry) is MultiLineString and (not lines or any(
                    len(line.vertices) < 2 or line.vertices.shape[1] != 3 for line in lines)):
                raise ValueError('CityJSON MultiLineString requires nonempty lines with at least two 3D vertices')
            surfaces = [] if type(geometry) in (Point, Mesh, MultiLineString) else geometry.surfaces
            if type(geometry) not in (Point, Mesh, MultiLineString) and (not surfaces or any(not len(s.vertices) for s in surfaces)):
                raise ValueError('CityJSON MultiSurface boundaries must be nonempty')
            for item in (geometry, *surfaces, *lines):
                if item is not geometry and type(geometry) is not MultiLineString and item.normal.size:
                    raise NotImplementedError('Stored Surface normals have no explicit CityJSON mapping')
                if item.fields or (item is not geometry and item.regions):
                    raise NotImplementedError('Nested fields/regions require an explicit CityJSON mapping')
