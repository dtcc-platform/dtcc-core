...
WATER_SURFACE_TYPES = frozenset(
    {"WaterSurface", "WaterGroundSurface", "WaterClosureSurface"}
)
SURFACE_TYPES = (
    BUILDING_SURFACE_TYPES | TRANSPORTATION_SURFACE_TYPES | WATER_SURFACE_TYPES
)
FEATURE_SURFACE_TYPES = {
    "Building": BUILDING_SURFACE_TYPES,
    "BuildingPart": BUILDING_SURFACE_TYPES,
    **{
...
def validate_region_owner(regions, feature_type):
    ...
    for region in regions:
        if (
            not region.semantic_type.startswith(SEMANTIC_NAMESPACE)
            or region.semantic_type[len(SEMANTIC_NAMESPACE) :] not in allowed
        ):
            raise NotImplementedError(
                f"{feature_type} semantic regions require its mapped external vocabulary"
            )
...
def read_regions(semantics, count):
    ...
    for entity, members in zip(entities, indices):
        ...
        if not isinstance(name, str) or name not in SURFACE_TYPES:
            raise NotImplementedError(
                "Strict CityJSON currently requires unextended semantic type names"
            )
        attrs = read_attributes(
            {
                key: value
                for key, value in entity.items()
                if key not in ("type", "parent", "children")
            },
            name,
        )
    ...
    for index, entity in enumerate(entities):
        ...
        for child in children:
            ...
            if region.parent is not None and region.parent != index:
                raise ValueError(
                    "Conflicting CityJSON semantic parent/children relationships"
                )
            region.parent = index
    actual_children = {}
...
def write_regions(regions, count):
    ...
    for region in regions:
        if not region.semantic_type.startswith(SEMANTIC_NAMESPACE):
            raise NotImplementedError(
                "Semantic URI has no CityJSON mapping in this profile"
            )
        ...
        if region.id is not None:
            raise NotImplementedError(
                "Region IDs require a defined CityJSON extension mapping"
            )
        ...
        if any(
            ...
            for value in region.attributes.values()
        ):
            raise ValueError("CityJSON semantic attributes must be scalar values")
