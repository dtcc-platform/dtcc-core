...
def load_3dbag(path, *, extent_policy="validate", validate_schema=True):
    ...
    city = load_cityjson(
        path, strict=True, extent_policy=extent_policy, validate_schema=False
    )
    if not city.transform.srs or CRS.from_user_input(city.transform.srs) != CRS(7415):
        raise ValueError(
            "3DBAG b3_h_dak mapping requires source CRS EPSG:7415 (RD New + NAP)"
        )
    seen = 0
    mapped_objects = 0
    stack = [city]
    while stack:
        ...
        if any(
            key in attrs for key in ("b3_h_50p", "b3_h_70p", "b3_h_min", "b3_h_max")
        ):
            raise ValueError(
                f"objects[{obj.id!r}]: newer 3DBAG elevation names are outside the b3_h_dak mapping"
            )
        elevations = []
        for key in _ELEVATIONS:
            ...
            if type(value) not in (int, float) or (
                ...
                raise ValueError(
                    f"objects[{obj.id!r}].attributes[{key!r}] must be a finite number or null"
                )
            elevations.append(
                {
                    "value": value,
                    "unit": "m",
                    "vertical_reference": NAP_CRS,
                    "reference": {
                        "value": key,
                        "code_space": MAPPING_ID + "/elevation-references",
                    },
        ...
        if roof is not None and type(roof) is not str:
            raise ValueError(
                f"objects[{obj.id!r}].attributes['b3_dak_type'] must be a string or null"
            )
        if roof in _ROOF_CODES:
            additions["roof_type"] = {
                "value": roof,
                "code_space": MAPPING_ID + "/roof-types",
            }
        for key in additions:
            if key in attrs:
                raise ValueError(
                    f"objects[{obj.id!r}].attributes[{key!r}]: 3DBAG mapping destination already exists"
                )
        attrs.update(additions)
        mapped_objects += bool(additions)
    if not seen:
        raise ValueError(
            "No supported 3DBAG b3_h_dak/ground elevation attributes found"
        )
    if validate_schema:
