#ifndef DTCC_VOLUME_MESH_REFINEMENT_H
#define DTCC_VOLUME_MESH_REFINEMENT_H
...
private:
  ...
  static std::vector<std::pair<size_t,size_t>> get_candidate_vertices_pairs(const VolumeMesh &volume_mesh,
    ...
    for (auto c : tetrahedra_to_remove)
    {
      ...
      // rv1 should always have marker greater or equal to rv2
      if (volume_mesh.markers[rv1] > volume_mesh.markers[rv2])
        std::swap(rv1, rv2);

      vertex_pairs.push_back({rv1, rv2});

      // std::cout << "Merging pair: " << rv1 <<" (" << volume_mesh.markers[rv1]<<" ) | " << rv2 <<
      // " (" << volume_mesh.markers[rv2]<<" )" << std::endl;
    }
