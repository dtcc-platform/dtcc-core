  ...
  public:
                              ...
                              bool sort_triangles = true)
    {
      ...
      for (const auto &v : surface.vertices)
      {
        ...
        if (projected_surface.size() == 0 || (!projected_v.close_to(projected_surface.front()) && !projected_v.close_to(projected_surface.back())))
      ...
      //    info("projected_surface area: " + str(Geometry::polygon_area(Polygon(projected_surface))));
      //    info("projected_surface size: " + str(projected_surface.size()));
      //
      ...
      for (const auto &hole : surface.holes)
      {
        ...
        for (const auto &v : hole)
        {
          ...
          auto e_v_prime = transform * e_v;
          auto projected_v = Vector2D(e_v_prime.x(), e_v_prime.y());
          if (projected_hole.size() == 0 || (!projected_v.close_to(projected_hole.front()) && !projected_v.close_to(projected_surface.back())))
