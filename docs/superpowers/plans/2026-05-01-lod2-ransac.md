...
## Task 0: Verify Existing API Surface

**Files:**
- Read: `dtcc_core/model/geometry/surface.py`
- Read: `dtcc_core/model/object/object.py`
- Read: `dtcc_core/builder/geometry_builders/buildings.py`
- Read: `dtcc_core/model/mixins/city/builder_mixin.py`

- [ ] **Step 1: Confirm geometry constructors and methods**

Run: `rg -n "class Surface|class MultiSurface|vertices:|surfaces:|holes:|def copy\\(|def from_polygon|def to_polygon" dtcc_core/model/geometry/surface.py`

Expected: output confirms `Surface` has `vertices` and `holes`, `MultiSurface` has `surfaces`, `Surface.copy(geometry_only=False)`, `Surface.from_polygon(polygon, height=0.0)`, and `Surface.to_polygon(...)`.

- [ ] **Step 2: Confirm building geometry methods and point-cloud property**

Run: `rg -n "def remove_geometry|def add_geometry|def point_cloud|def pointcloud" dtcc_core/model/object/object.py`

Expected: output confirms `add_geometry`, `remove_geometry`, `point_cloud`, and `pointcloud` exist.

- [ ] **Step 3: Confirm roof-point sourcing behavior**

Run: `rg -n "def building_heights_from_pointcloud|keep_roof_points|extract_roof_points|remove_geometry\\(GeometryType.POINT_CLOUD\\)" dtcc_core/builder/geometry_builders/buildings.py`

Expected: output confirms `building_heights_from_pointcloud` accepts `keep_roof_points`, calls `extract_roof_points`, and removes `GeometryType.POINT_CLOUD` only when `keep_roof_points` is false.

- [ ] **Step 4: Confirm LOD1 wrapper conventions**

Run: `sed -n '65,180p' dtcc_core/model/mixins/city/builder_mixin.py`

Expected: output shows `CityBuilderMixin.build_lod1_buildings`, including terrain generation, `building_heights_from_pointcloud`, and replacement of city buildings after builder calls.

- [ ] **Step 5: Stop if any assumption is false**

If any expected API is missing or has a different signature, update this plan before starting Task 1. Do not compensate silently during implementation.
...
def _multi_plane_roof_surfaces(points: np.ndarray, footprint: Polygon, planes: list[RoofPlane]) -> list[Surface] | None:
    if len(planes) != 2:
        # First prototype supports flat and two-plane gable-like roofs only.
        return None
...
Run: `pytest tests/builder/test_lod2_buildings.py::test_city_build_lod2_buildings_uses_existing_roof_points_without_recomputing_heights tests/builder/test_lod2_buildings.py::test_city_build_lod2_buildings_calculates_heights_and_keeps_roof_points -v`
...
Expected: PASS for both wrapper tests.
