...
sys.modules.setdefault("footprints_table_case", script)
...
def test_output_dir_is_under_output_footprints_table_case():
    assert script.OUTPUT_DIR == Path("output/footprints/table_case")
...
def test_case_is_table_footprints_geojson():
    assert script.CASE["name"] == "footprints_geojson"
    assert script.CASE["filename"] == "building_footprints.geojson"
    assert script.CASE["dataset_key"] == "table-footprints-geojson"
    assert _SERVER_DATASET_KEY_RE.fullmatch(script.CASE["dataset_key"])
    assert script.CASE["publish_to_atlas"] is True


def test_case_requests_sweref99_tm_geojson():
    """The table validator rejects anything but EPSG:3006 GeoJSON."""
    assert script.CASE["params"]["crs"] == "EPSG:3006"
...
def test_run_case_purges_stale_case_dir(tmp_path):
    case_dir = tmp_path / "footprints_geojson"
    case_dir.mkdir()
...
def test_run_case_calls_export_with_bounds_title_and_crs(tmp_path):
    dataset = MagicMock()
    dataset.export.return_value = MagicMock()
    ...
    assert call.args[0] == (
        tmp_path / "footprints_geojson" / "building_footprints.geojson"
    )
...
def test_main_exports_without_publish(tmp_path, capsys):
    ...
    assert (tmp_path / "footprints_geojson").is_dir()
...
def test_main_publishes_when_creds_set(tmp_path, capsys):
    package = MagicMock()
    package.publish.return_value = SimpleNamespace(
        dataset_key="table-footprints-geojson", version_number=1
    )
    dataset = MagicMock()
    dataset.export.return_value = package
