...
def test_public_reprojection_preserves_source_and_native_arrays():
    ...
    assert (
        result.transform is not source.transform and result.transform.srs == "EPSG:4326"
    )
    ...
    assert exchange.dumps(exchange.loads(exchange.dumps(result))) == exchange.dumps(
        result
    )
    source.transform.srs = ""
    assert (
        reproject_pointcloud(source, "EPSG:3006", "EPSG:4326").transform.srs
        == "EPSG:4326"
    )
...
def test_reprojection_rejects_state_without_a_transformation_rule():
    ...
    with pytest.raises(TypeError, match="override_geometry_crs"):
        reproject_pointcloud(
            source, "EPSG:32633", "EPSG:4326", override_geometry_crs="false"
        )
    source.fields = [
        Field(name="velocity", association="sample", values=np.array([2.0]))
    ]
    before = exchange.dumps(source)
    ...
    integer_source = PointCloud(
        points=np.array([[500000, 6500000, 2**53 + 1]], dtype=np.uint64)
    )
    with pytest.raises(ValueError, match="integer Z"):
        reproject_pointcloud(integer_source, "EPSG:3006", "EPSG:4326")
    surface = Surface(
        vertices=np.vstack(
            (source.points, source.points + [1, 0, 0], source.points + [0, 1, 0])
        ),
        fields=[Field(name="temperature", association="face", values=np.array([2.0]))],
    )
    obj = Object(id="o")
    ...
    with pytest.raises(NotImplementedError, match="Tree object reprojection"):
        reproject_object(
            Tree(position=np.array([500000.0, 6500000.0, 100.0])),
            "EPSG:3006",
            "EPSG:4326",
        )
