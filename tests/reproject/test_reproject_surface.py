...
class TestReprojectionBasics:
    ...
    def test_z_coordinates_preserved(self, basic_surface):
        ...
        np.testing.assert_array_equal(
            result.vertices[:, 2], basic_surface.vertices[:, 2]
        )
    ...
    def test_multiple_holes_preserved(self, surface_with_multiple_holes):
        ...
        result = reproject_surface(
            surface_with_multiple_holes, "EPSG:3006", "EPSG:4326"
        )
    ...
    def test_holes_coordinates_change(self, surface_with_holes):
        ...
        assert not np.allclose(
            result.holes[0][:, :2], surface_with_holes.holes[0][:, :2]
        )
...
class TestCoordinateAccuracy:
    ...
    def test_round_trip_transformation(self, basic_surface):
        ...
        np.testing.assert_allclose(
            result.vertices, basic_surface.vertices, rtol=1e-5, atol=1e-3
        )
    ...
    def test_round_trip_with_holes(self, surface_with_holes):
        ...
        np.testing.assert_allclose(
            result.vertices, surface_with_holes.vertices, rtol=1e-5, atol=1e-3
        )
        ...
        for i in range(len(result.holes)):
            np.testing.assert_allclose(
                result.holes[i], surface_with_holes.holes[i], rtol=1e-5, atol=1e-3
            )
...
class TestNormalPreservation:
    ...
    def test_normal_requires_explicit_transformation(self, basic_surface):
        ...
        np.testing.assert_array_equal(basic_surface.normal, [0.0, 0.0, 1.0])
