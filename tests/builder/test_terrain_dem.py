...
def test_ground_raster_retains_zero_negative_and_crs_with_explicit_gaps():
    ...
    raster = builder.build_terrain_raster(
        pc, 1., Bounds(0, 0, 3.2, 3.2), window_size=0, radius=.1, hole_fill='none',
    )
    ...
    assert raster.georef * (.5, 3.5) == (.5, .5)
    ...
    assert np.isfinite(filled.data).all() and set(np.unique(filled.data)) == {0., -2.}
    assert np.isnan(raster.data).sum() == 14  # Source was not modified.
    all_points = builder.build_terrain_raster(
        pc, 1., Bounds(0, 0, 3.2, 3.2), window_size=0, radius=.1,
        hole_fill='none', ground_only=False,
    )
    assert 99. in all_points.data
...
def test_no_ground_or_support_fails_instead_of_substituting_points():
    ...
    with pytest.raises(ValueError, match='class 2'):
        builder.build_terrain_raster(pc, 1.)
    pc.classification = np.array([], dtype=np.uint8)
    with pytest.raises(ValueError, match='classification'):
        builder.build_terrain_raster(pc, 1.)
    with pytest.raises(ValueError, match='support'):
        builder.build_terrain_raster(source(), 1., Bounds(10, 10, 12, 12), window_size=0)
...
def test_dem_metadata_and_arrays_survive_native_package_and_reject_invalid_write(tmp_path):
    terrain = make_dem()
