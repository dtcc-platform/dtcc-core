...
@pytest.fixture
def geometry():
    path = Path(__file__).resolve().parents[2] / 'sandbox/model_profiles/fixtures/buildings.city.json'
...
def test_mesh_preserves_regions_holes_and_unclassified_faces(geometry, mesher, tmp_path):
    ...
    assert [r.semantic_type for r in mesh.regions] == [r.semantic_type for r in geometry.regions]
...
def test_meshing_reports_unmapped_operations_before_mutating_input(geometry):
    ...
    geometry.fields = [Field(name='temperature', values=np.zeros(len(geometry.surfaces)), association='face')]
...
@pytest.fixture
def solid():
    vertices = np.array([[0, 0, 0], [4, 0, 0], [4, 4, 0], [0, 4, 0],
                         [0, 0, 4], [4, 0, 4], [4, 4, 4], [0, 4, 4]], dtype=float)
    ...
    value = Solid(
        ...
        regions=[SemanticRegion('https://github.com/dtcc-platform/dtcc-core/schemas/model#WallSurface',
                                indices=np.array([2, 3, 8, 9]), id='walls',
...
def test_solid_mesh_includes_cavity_and_preserves_source(solid, mesher, tmp_path):
    ...
    mesh.fields = [Field(name='triangle_area', unit='m2', association='face', values=areas(mesh))]
    mesh.save(tmp_path / 'boundary.dtcc')
    assert exchange.dumps(io.load_model(tmp_path / 'boundary.dtcc')) == exchange.dumps(mesh)
...
@pytest.mark.skipif('dtcc_mesher' not in available_2d_meshers(), reason='dtcc_mesher not installed')
def test_nearly_collinear_leading_vertices_do_not_tilt_mesh_projection():
    ...
    vertices = np.array([[0, 0, 0], [5, 0, .001], [6, .001, .001],
                         [10, 0, 0], [10, 10, 0], [0, 10, 0]], dtype=float)
    ...
    assert np.ptp(mesh.vertices[:, 2]) < .002
