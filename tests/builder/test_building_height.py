...
def test_city_lod1_separates_measurement_default_and_clamp_through_io(tmp_path):
    ...
    city.build_lod1_buildings(calculate_heights=False, always_use_default=True,
                             default_ground_height=100, min_building_height=2.5)
...
def test_pointcloud_estimate_preserves_measurement_and_handles_missing_points():
    ...
    observed.add_geometry(PointCloud(points=np.array([[1., 1., 108.], [1., 1., 108.]])),
                          GeometryType.POINT_CLOUD)
    missing = building('missing', box(2, 0, 4, 2))
    terrain = Raster(data=np.full((5, 5), 100.))
