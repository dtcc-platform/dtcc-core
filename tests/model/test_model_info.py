...
def test_all_concrete_models_share_display_contract(capsys):
    classes = {getattr(model, name) for name in model.__all__
...
def test_info_print_return_and_file_are_identical(capsys):
    mesh = Mesh(vertices=np.array([[0., 0., 0.], [1., 0., 0.], [0., 1., 0.]]),
                ...
                fields=[Field(name='temperature', unit='°C', association='vertex',
...
def test_sensor_details_and_vehicle_guidance_live_in_info():
    ...
    station.add_geometry(Point(x=1, y=2, fields=[Field(name='temperature', unit='°C',
                         values=np.array([21.0]))]), id='location')
    ...
    vehicles = VehicleCollection(attributes={
        'partial_result': True,
        'configuration_help': ['Set [API_KEY] to enable the provider.'],
        'upstream_errors': [{'failure_class': 'credentials', 'message': 'Missing key'}],
    })
