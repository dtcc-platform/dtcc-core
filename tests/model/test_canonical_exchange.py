...
def city():
    ...
    building = Building(
        id="building",
        semantic_type="https://example.org/Building",
        attributes={
            "height": 12.5,
            "integer": 2**80,
            "float": 1.0,
    ...
    root.dataset_context = DatasetContext(
        ...
        provenance=DatasetProvenance(sources=["example"]),
        presentation=DatasetPresentation(),
        request=DatasetRequest(dataset_name="example", bounds=[0.0, 0.0, 1.0, 1.0]),
        health={"status": "ok"},
        warnings=["synthetic"],
    )
    return root
...
def test_public_file_round_trip_preserves_semantic_and_computational_state(
    city, tmp_path
):
...
def test_wire_boundary_rejects_unsupported_or_malformed_payload(city, failure):
    ...
    if failure == "version":
        ...
        pb.object.children[0].children[0].representations[
            0
        ].geometry.mesh.vertices.data = b"bad"
...
def test_canonical_publication_passes_a_complete_package_to_the_uploader(city):
    ...
    assert city.publish(
        dataset_key="canonical-city", canonical=True, uploader=Uploader()
    ) == {
...
def test_requested_supplement_keeps_canonical_artifact(city, tmp_path):
    ...
    assert {artifact.role for artifact in package.artifacts} == {
        "canonical_model",
        "derived",
    }
    ...
    np.testing.assert_array_equal(
        load_model_package(package.path).vertices, mesh.vertices
    )


@pytest.mark.parametrize("failure", ["hash", "type", "spatial", "path"])
def test_package_reader_validates_manifest_against_actual_payload(
    city, tmp_path, failure
):
    ...
    if failure == "hash":
        ...
        artifact["bounds"] = [0.0, 0.0, 1.0, 1.0]
    else:
...
def test_package_supplement_failure_preserves_previous_archive(
...
def test_double_precision_overflow_is_rejected_before_file_replacement(
    tmp_path, target
):
    point = Point()
    if target == "point":
...
def test_repeated_and_high_cardinality_metadata_remain_independently_editable():
    ...
    for id in ("first", "second"):
        root.add_child(
            Object(id=id, attributes={"label": "Göteborg", "nested": ["same", ""]})
        )
    root.add_child(
        Object(id="many", attributes={"names": [f"name-{i}" for i in range(6000)]})
    )
