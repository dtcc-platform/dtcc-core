...
def test_height_has_one_stored_authority_and_no_fallback(cls, tmp_path):
    ...
    value.add_geometry(Surface(vertices=np.array([[0., 0., 0.], [2., 0., 0.], [0., 2., 3.]])),
                       GeometryType.LOD2)
    assert value.height is None and value.measured_height is None
    assert value.lod2.bounds.depth == 3  # An explicitly selected local-coordinate extent.
    value.height = 12.5
...
def test_estimate_validation_occurs_at_persistence_and_preserves_file(tmp_path):
    ...
    assert value.estimated_height == -1 and value.height == 12.5  # Cheap access while editing.
