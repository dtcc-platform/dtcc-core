...
def historical_city(path):
    ...
    while pending:
        obj = pending.pop()
        pending.extend(child for children in obj.children.values() for child in children)
        ...
        for representation in obj.geometry.values():
            for region in getattr(representation.geometry, 'regions', []):
                if region.semantic_type.startswith(SEMANTIC_NAMESPACE):
                    region.semantic_type = NS + region.semantic_type[len(SEMANTIC_NAMESPACE):]
    return city
...
def test_region_host_diagnostic_and_native_structure_boundary(profile):
    ...
    ground = next(i for i, r in enumerate(geom.regions) if r.semantic_type == NS + 'GroundSurface')
    geom.regions[1].parent = ground  # Structurally valid, semantically wrong host.
    report = profile.validate(city)
    assert [(i.path, i.rule) for i in report.issues] == [
        ("objects['part-1'].geometry['cityjson-0'].geometry.regions[1].parent", 'target_type')]
...
def test_edited_schema_requires_values_and_preserves_loaded_version(profile, tmp_path):
    ...
    assert {i.path for i in issues} == {
        "objects['bench-1'].attributes['charging_power']", "objects['bench-1'].attributes['operator']",
    ...
    schema['classes']['ChargingBench']['attributes']['charging_power']['maximum_value'] = 50
    path.write_text(yaml.safe_dump(schema))
...
def test_typed_field_authority_and_path(profile, tmp_path):
    ...
    city.add_child(tree)
    assert SemanticProfile(path).validate(city).issues[0].path == "objects['tree'].crown_radius"
    tree.attributes['height'] = 4
    with pytest.raises(ValueError, match=r"objects\['tree'\].attributes\['height'\].*conflicts"):
        profile.validate(city)
