...
from dtcc_core.model import (Object, City, Building, GeometryType, GeometryRepresentation,
                             Point, Surface, Solid, SemanticRegion, Field, exchange)
from dtcc_core.model import dtcc_pb2 as wire
from dtcc_core.io.cityjson.cityjson import load


def solid_document():
    cube = [[0,0,0],[4,0,0],[4,4,0],[0,4,0],[0,0,4],[4,0,4],[4,4,4],[0,4,4]]
    inner = [[1+x//2,1+y//2,1+z//2] for x,y,z in cube]
    faces = [[0,3,2,1],[4,5,6,7],[0,1,5,4],[1,2,6,5],[2,3,7,6],[3,0,4,7]]
    geometry = {'type':'Solid', 'lod':'2.2',
                'boundaries': [[[f] for f in faces], [[[i+8 for i in reversed(f)]] for f in faces]],
                'semantics': {'surfaces': [{'type':'WallSurface','material':'brick'}],
                              'values': [[None,None,0,0,0,0],[None,None,0,0,0,0]]}}
    return {'type':'CityJSON','version':'2.0','transform': {'scale':[1,1,1],'translate':[0,0,0]},
            'vertices':cube+inner, 'CityObjects':{'building':{'type':'Building','geometry':[geometry]}}}
...
def test_solid_shells_regions_fields_and_transforms_roundtrip(tmp_path):
    ...
    solid.fields = [Field(name='flux', association='face', values=np.arange(12, dtype=float))]
...
def test_shells_must_partition_surfaces(failure):
    solid = load(solid_document(), strict=True).buildings[0].get_geometry(lod='2.2')
    if failure == 'duplicate': solid.shells[1][0] = 0
    elif failure == 'uncovered': solid.shells.pop()
    else: solid.shells[1][0] = 12
...
def test_cityjson_repeated_lod_and_interior_shell_roundtrip(tmp_path):
    ...
    source['CityObjects']['building']['geometry'].append(deepcopy(source['CityObjects']['building']['geometry'][0]))
...
def test_parent_bounds_do_not_mutate_child_or_first_surface_bounds():
    ...
    for offset in (0.,100.):
        building = Building()
        building.add_geometry(Surface(vertices=np.array([[offset,0.,0.],[offset+1,0.,0.],[offset,1.,0.]])),GeometryType.LOD0)
...
def test_cityjson_preserves_existing_degeneracy_but_rejects_new_quantization_loss(tmp_path):
    source = solid_document()
