...
def terrain_with_elevation():
    ...
    terrain.add_geometry(Raster(data=np.array([[0.0, -2.0], [1.0, 3.0]])), id="dem")
    terrain.attributes["elevation_rasters"] = [
        {
            "geometry_id": "dem",
            "unit": "m",
            "vertical_reference": "EPSG:5613",
            "sampling": "cell_center",
            "interpolation": "inverse_distance_weighted",
            "hole_fill": "none",
            "source_selection": "ground",
            "window_size": 0,
            "radius": 0.0,
        }
    ]
...
def test_local_representation_link_checked_at_default_save_load_and_bypass(tmp_path):
    ...
    for expected_rule in ("dangling_representation", "representation_type"):
        ...
        assert [(issue.path, issue.rule) for issue in issues] == [
            (
                "objects['terrain'].attributes['elevation_rasters'][0]['geometry_id']",
                expected_rule,
            )
        ]
...
def test_reference_annotation_is_schema_driven_and_rejects_invalid_configuration(
    tmp_path,
):
    ...
    schema["classes"]["Note"] = {
        "attributes": {
            "representation": {
                "range": "string",
                "annotations": {"dtcc_representation_reference": "Raster"},
            }
        }
    }
    schema["classes"]["Terrain"]["attributes"]["note"] = {
        "range": "Note",
        "inlined": True,
    }
    ...
    assert all(
        i.rule != "dangling_representation" for i in profile.validate(terrain).issues
    )
    assert not profile.validate(terrain).valid
    for target in ("Typo", "Geometry"):
        schema["classes"]["Note"]["attributes"]["representation"]["annotations"][
            "dtcc_representation_reference"
        ] = target
        path.write_text(yaml.safe_dump(schema))
    ...
    schema["classes"]["Field"]["attributes"]["note"] = {
        "range": "Note",
        "inlined": True,
    }
    path.write_text(yaml.safe_dump(schema))
...
def test_source_region_host_survives_exchange_and_wrong_theme_fails(tmp_path):
    ...
    assert (
        encoded["CityObjects"]["road"]["geometry"][1]["semantics"]["surfaces"]
        == surfaces
    )
    ...
    assert any(
        i.path == "geometry.regions[2].parent" and i.rule == "target_type"
