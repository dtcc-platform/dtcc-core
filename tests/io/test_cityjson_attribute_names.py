...
def test_mixed_city_names_survive_external_and_canonical_boundaries(tmp_path):
    ...
    objects['building-1']['attributes'].update(
        measuredHeight=12.5, estimated_height=8.0, roofType='gable', customLabel={'measuredHeight': 'untouched', 'storeysAboveGround': 'untouched'})
    plant = next(value for value in objects.values() if value['type'] == 'SolitaryVegetationObject')
    plant.setdefault('attributes', {}).update(crownDiameter=4.0, trunkDiameter=0.5)
    bench = next(value for value in objects.values() if value['type'] == 'CityFurniture')
    bench.setdefault('attributes', {}).update(measuredHeight='unrelated user metadata',
    ...
    assert attrs['customLabel'] == {'measuredHeight': 'untouched', 'storeysAboveGround': 'untouched'}
    native_plant = next(obj for obj in city.get_children(Object) if obj.semantic_type.endswith('#SolitaryVegetationObject'))
    ...
    assert {key: value.get('attributes', {}) for key, value in exported['CityObjects'].items()} == {
...
def test_ordinary_and_strict_building_names_reject_lossy_collisions(tmp_path, strict):
    ...
    assert json.loads(target.read_text())['CityObjects']['building-1']['attributes']['measuredHeight'] == 10.2
...
def test_retired_standard_versions_and_generic_metadata_are_not_migrated(tmp_path):
    value = Building(id='b', attributes={'measuredHeight': 'user metadata', 'customLabel': 7})
    path = tmp_path / 'building.dtcc'
    io.save_model(value, path)
    assert io.load_model(path).attributes == value.attributes  # No global case conversion.
    ...
    assert restored.schema_version == '0.2.0' and restored.attributes == value.attributes
