...
def source():
    ...
    fixture = Path(__file__).parents[2] / 'sandbox/model_profiles/fixtures/buildings.city.json'
    data = json.loads(fixture.read_text())
    data['metadata']['referenceSystem'] = 'https://www.opengis.net/def/crs/EPSG/0/7415'
    data['CityObjects']['building-1']['attributes'] = {
        'b3_h_maaiveld': -0.41, 'b3_h_dak_50p': 11.34,
        'b3_h_dak_min': None, 'b3_dak_type': 'multiple horizontal',
        'b3_pw_bron': 'AHN3', 'b3_pw_datum': 2014,
...
def test_mapping_public_boundaries_and_source_preservation(tmp_path):
    ...
    attrs = b.attributes
    assert data == before
    assert all(attrs[k] == v for k, v in data['CityObjects'][b.id]['attributes'].items())
...
def test_source_contract_failures_and_missing_values():
    ...
    for roof in ('unknown', 'no points', 'no planes', 'future_supplier_code', None):
        ...
        assert 'roof_type' not in b.attributes and 'elevation_measurements' not in b.attributes
...
def test_elevation_schema_applies_to_ordinary_save_load_and_bypass(tmp_path):
    ...
    for key, value in [('vertical_reference', ''), ('unit', 's'), ('value', True),
                       ('reference', {'value': 'ground'}), ('typo', 'extra')]:
        record.clear()
