...
FIXTURE = Path(__file__).resolve().parents[2] / 'sandbox/model_profiles/fixtures/openings.city.json'
...
def test_openings_public_workflow_and_meshing(tmp_path):
    ...
    assert exported['CityObjects']['part-1']['geometry'][0]['semantics'] == source['CityObjects']['part-1']['geometry'][0]['semantics']
    ...
    city.dataset_context = DatasetContext(
        ...
        presentation=DatasetPresentation(), request=DatasetRequest(dataset_name='openings'),
    )
    ...
    assert [areas[r.indices].sum() for r in mesh.regions[:3]] == pytest.approx([50, 4, 6])
    assert areas.sum() == pytest.approx(440)
    mesh.save(tmp_path / 'openings-mesh.dtcc')
    assert exchange.dumps(io.load_model(tmp_path / 'openings-mesh.dtcc')) == exchange.dumps(mesh)
...
def test_generic_parent_admission_wire_version_and_merge():
    ...
    assert exchange.dumps(exchange.loads(exchange.dumps(merged))) == exchange.dumps(merged)
    # Native checks are domain-independent; long valid hierarchies are iterative.
    generic = MultiSurface(regions=[SemanticRegion('urn:example:region', parent=i-1 if i else None)
                                   for i in range(2000)])
