...
def test_numerical_and_object_state_share_one_message(tmp_path):
    ...
    raster = model.Raster(
        data=np.array([[[2**64 - 1], [2**53 + 1]]], dtype=np.uint64),
        georef=Affine(2, 0.25, 100, 0.5, -2, 200),
        crs="EPSG:3006",
    )
...
def test_new_file_wrappers_and_failed_read_preserve_receiver(tmp_path):
    ...
    for value, reader in [
        (cloud, io.load_pointcloud),
        (mesh, io.load_volume_mesh),
        (raster, io.load_raster),
    ]:
...
def test_malformed_value_metadata_fails_before_replacing_native_state():
    field = model.Field(name="original", association="sample", values=np.array([2.0]))
    ...
    raster = model.Raster(data=np.array([[2.0]]), crs="EPSG:3006")
