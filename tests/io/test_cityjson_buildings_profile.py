...
from dtcc_core.model import Field, GeometryType, Mesh, MultiSurface, SemanticRegion, Surface
from dtcc_core.io.cityjson import cityjson
from dtcc_core.io.cityjson.semantics import SEMANTIC_NAMESPACE

FIXTURE = Path(__file__).resolve().parents[2] / 'sandbox/model_profiles/fixtures/buildings.city.json'
...
def test_public_buildings_workflow_preserves_identity_geometry_and_regions(tmp_path):
    ...
    assert exported['CityObjects']['part-1']['geometry'][0]['semantics']['values'] == [0,1,1,2,2,2,None]
    ...
    for original, final in zip(part.lod2.surfaces, restored.buildings[0].building_parts[0].lod2.surfaces):
        np.testing.assert_allclose(original.vertices, final.vertices, atol=0.0005, rtol=0)
        for a, b in zip(original.holes, final.holes):
            np.testing.assert_allclose(a, b, atol=0.0005, rtol=0)


@pytest.mark.parametrize('failure', ['index', 'semantic_index', 'solid', 'invalid_lod', 'parent', 'appearance', 'extent', 'boolean_vertex'])
...
def test_all_lods_and_nested_part_ids_survive(source, tmp_path):
    ...
    source['CityObjects']['building-1']['geometry'].append(deepcopy(child['geometry'][0]))
...
def test_canonical_surface_preserves_nested_metadata_and_requires_v2():
    surface = Surface(vertices=np.array([[0.,0.,0.],[3.,0.,0.],[0.,3.,0.]]), normal=np.array([0.,0.,1.]))
    surface.transform.set_translation(1.125, 0, 0)
    surface.add_field(Field(name='flux', association='face', values=np.array([11.25])))
    source = MultiSurface(surfaces=[surface], regions=[SemanticRegion('urn:example:roof', np.array([0]), id='roof')])
    data = exchange.dumps(source)
    result = exchange.loads(data)
    np.testing.assert_array_equal(result.surfaces[0].transform.affine, surface.transform.affine)
...
def test_region_indices_remain_generic_for_mesh_faces():
    mesh = Mesh(vertices=np.array([[0.,0.,0.],[1.,0.,0.],[0.,1.,0.]]), faces=np.array([[0,1,2]]))
...
def test_missing_geometry_and_unclassified_surfaces_remain_unclassified(source, tmp_path):
...
def test_empty_multisurface_is_canonical_but_not_cityjson(source, tmp_path):
    source['CityObjects']['building-1']['geometry'] = [{'type':'MultiSurface', 'lod':'0', 'boundaries':[]}]
...
def test_multisurface_merge_offsets_regions_without_mutating_source():
    polygon = Surface(vertices=np.array([[0.,0.,0.],[1.,0.,0.],[0.,1.,0.]]))
    left = MultiSurface(surfaces=[polygon] * 300)
    right = MultiSurface(surfaces=[polygon.copy()], regions=[SemanticRegion('urn:roof', np.array([0], dtype=np.uint8))])
    left.merge(right)
...
def test_cxx_boundary_reports_unsupported_regions_before_conversion():
    from dtcc_core.builder.model_conversion import create_builder_multisurface, mesh_to_builder_mesh
