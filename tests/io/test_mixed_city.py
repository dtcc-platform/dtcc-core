...
from dtcc_core.model import City, Object, Tree, Landuse, Point, exchange, dtcc_pb2 as wire
from dtcc_core.model.object.landuse import LanduseClasses

FIXTURE = Path(__file__).resolve().parents[2] / 'sandbox/model_profiles/fixtures/mixed-city.city.json'
...
def test_native_tree_landuse_state_is_exact_and_independently_mutable(tmp_path):
    tree = Tree(id='tree', position=np.array([325000.123456789, 6400000.987654321, 12.125]),
                height=8.123456789, crown_radius=2.123456789, semantic_type=NS + 'Tree')
    ...
    park = Landuse(id='park', landuses=[LanduseClasses.GRASS, LanduseClasses.UNKNOWN, LanduseClasses.GRASS])
    city = City(id='city')
    ...
    assert tree.position[0] == 325000.123456789 and tree.attributes['source']['labels'] == ['oak']
    assert len(park.landuses) == 3
    # Intentional native defaults and exact integer positions are not normalized.
    for value in (Tree(), Tree(position=np.empty(0)), Tree(position=np.array([2**60+1, 2, 3], dtype=np.uint64)), Landuse()):
        assert exchange.dumps(exchange.loads(exchange.dumps(value))) == exchange.dumps(value)
...
@pytest.mark.parametrize('value', [Tree(position=np.zeros(2)), Tree(position=np.array([np.nan, 0, 0])),
    Tree(height=-1), Tree(crown_radius=np.inf), Tree(height=True), Landuse(landuses=['GRASS'])])
...
@pytest.mark.parametrize('failure', ['point_count', 'point_index', 'point_field', 'landuse_solid', 'feature_child', 'feature_type', 'region'])
def test_strict_source_rejects_unsupported_or_invalid_state(failure):
    ...
    if failure == 'point_count':
        ...
        source['CityObjects']['park-1']['geometry'][0]['semantics'] = {'surfaces': [{'type':'GroundSurface'}], 'values':[0]}
