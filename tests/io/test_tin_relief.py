...
def test_public_mixed_terrain_roundtrip_and_direct_access(tmp_path):
    ...
    assert terrain.semantic_type == 'https://github.com/dtcc-platform/dtcc-core/schemas/model#TINRelief'
    ...
    coordinates = np.asarray(data['vertices']) * data['transform']['scale'] + data['transform']['translate']
    for t in city.get_children(Terrain):
        ...
        for record, g in zip(t.geometry.values(), original['geometry']):
            np.testing.assert_array_equal(record.geometry.vertices[record.geometry.faces],
    ...
    city.dataset_context = DatasetContext(
        identity={'name': 'test-tin', 'title': 'Synthetic TIN'}, metadata={},
        provenance={}, presentation={}, request={'dataset_name': 'test-tin'})
    ...
    for t in city.get_children(Terrain):
        assert all(g['type'] == 'CompositeSurface' and 'semantics' not in g
...
def test_malformed_or_unsupported_tin_is_never_partially_imported():
    ...
    for boundaries in ([], [[[]]], [[[0, 1, 2, 3]]], [[[0, 1, 2], [3, 4, 5]]],
                       [[[0, 0, 1]]], [[[True, 1, 2]]], [[[-1, 1, 2]]], [[[0, 1, 99999]]]):
        data = source()
    ...
    for key, value in [('semantics', {'surfaces': [], 'values': [None]}), ('type', 'MultiSurface')]:
        data = source()
...
def test_unmapped_mesh_state_and_generic_terrain_do_not_overwrite(tmp_path):
    ...
    for field, value, message in [
        ...
        ('fields', [Field(name='z', unit='m', association='vertex', values=np.arange(4.))], 'fields'),
        ('regions', [SemanticRegion('https://github.com/dtcc-platform/dtcc-core/schemas/model#GroundSurface', np.array([0]))], 'semantic regions'),
        ('vertices', np.vstack([west(city).get_geometry(lod='1.2').vertices, [1, 2, 3]]), 'Unused'),
    ]:
...
def test_quantization_collapse_and_geometry_edit(tmp_path):
    ...
    mesh.vertices[0, 2] += .123
    path = tmp_path / 'terrain.city.json'
    city.save(path, strict=True)
    verify(city, io.load_city(path, strict=True))
    before = path.read_bytes()
    mesh.vertices[1] = mesh.vertices[0] + [.00001, 0, 0]
