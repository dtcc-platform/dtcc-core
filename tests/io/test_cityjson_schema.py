...
def test_public_default_schema_and_bypass_preserve_files(source, tmp_path, suffix):
    ...
    assert (
        restored.buildings[0].building_parts[0].attributes["storeys_above_ground"]
        is True
    )
    ...
    assert (
        io.load_city(tmp_path / "bypassed.dtcc", validate_schema=False).schema_version
        == DEFAULT_VERSION
    )
...
def test_direct_dictionary_boundaries_and_open_metadata(source):
    ...
    assert (
        to_cityjson(city, strict=True)["CityObjects"]["building-1"]["attributes"][
            "custom"
        ]
        == attrs["custom"]
    )
...
def test_schema_bypass_does_not_bypass_format_or_native_integrity(source, tmp_path):
    ...
    malformed["CityObjects"]["building-1"]["geometry"][0]["boundaries"][0][0][0] = (
        999999
    )
...
def test_opening_host_is_semantic_but_dangling_host_is_structural():
    ...
    semantics["surfaces"][0]["type"] = (
        "GroundSurface"  # Children still refer to this host.
    )
...
def test_validation_option_cannot_claim_permissive_validation(source, tmp_path):
    ...
    for call in [
        lambda flag: io.load_city(path, validate_schema=flag),
