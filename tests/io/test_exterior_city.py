...
def feature(city, id):
    return next(
        child for group in city.children.values() for child in group if child.id == id
    )
...
def test_exterior_public_roundtrip_and_direct_access(tmp_path):
    ...
    np.testing.assert_array_equal(
        road.get_geometry(lod="0").linestrings[0].vertices,
        [[0, 0, 0], [2, 0, 0], [2, 2, 0]],
    )
    ...
    assert (
        feature(city, "water")
        .get_geometry(lod="2.2")
        .regions[0]
        .attributes["water_level"]
        == "meanWaterLevel"
    )
    ...
    for id, original in source()["CityObjects"].items():
        assert exported["CityObjects"][id]["attributes"] == original.get(
            "attributes", {}
        )
    ...
    for id in source()["CityObjects"]:
        assert exchange.dumps(feature(reloaded, id)) == exchange.dumps(
            feature(city, id)
        )
...
def test_owner_vocabulary_and_unmapped_values_rejected_even_with_bypass(tmp_path):
    ...
    data = source()
    data["CityObjects"]["road"]["geometry"][1]["semantics"]["surfaces"][0][
        "function"
    ] = ["carriageway"]
    ...
    feature(city, "road").get_geometry(id="cityjson-1").regions[0].semantic_type = (
        SEMANTIC_NAMESPACE + "WaterSurface"
    )
    with pytest.raises(NotImplementedError, match="semantic regions"):
...
def test_lines_reject_unrepresentable_state_and_quantization_without_overwrite(
    tmp_path,
):
    ...
    with pytest.raises(ValueError, match="3D vertices"):
        city.save(path, strict=True, validate_schema=False)
    assert path.read_bytes() == before
    lines.linestrings[0].vertices = np.array([[0.0, 0.0, 0.0], [0.00001, 0.0, 0.0]])
...
def test_plant_height_schema_failure_and_explicit_bypass(tmp_path):
    ...
    feature(city, "plants").attributes["average_height"] = -1.0
    for path in before:
    ...
    assert (
        feature(exchange.loads(payload, validate_schema=False), "plants").attributes[
            "average_height"
        ]
        == -1.0
    )
