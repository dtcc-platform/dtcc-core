...
def test_nested_record_errors_have_native_paths_and_do_not_coerce():
    ...
    for slot, value in cases:
        ...
        assert any(
            f"objects['part-1'].attributes['height_measurements'][0]['{slot}']"
            in issue.path
    ...
    for missing in ("value", "unit", "status", "high_reference", "low_reference"):
        ...
        part.attributes = {"height_measurements": [record]}
        assert any(
            issue.path.endswith(f"[0]['{missing}']") and issue.rule == "required"
            for issue in profile.validate(city).issues
        )
    for value in (
        {"value": "a"},
        {"value": 10, "code_space": "codes"},
        {"value": "a", "code_space": " "},
...
def test_invalid_edit_save_load_bypass_and_file_preservation(tmp_path):
    ...
    with pytest.raises(ValueError, match="unit"):
        io.load_city(path)
    assert (
        io.load_city(path, validate_schema=False).buildings[0].attributes
        == city.buildings[0].attributes
    )
...
def test_record_support_is_schema_driven_and_does_not_create_entity_graphs(tmp_path):
    ...
    schema["classes"]["AbstractBuilding"]["attributes"]["survey_note"] = {
        "range": "SurveyNote",
        "inlined": True,
    }
    ...
    building = Building(id="b", attributes={"survey_note": {"confidence": 0.8}})
    assert profile.validate(building).valid
    building.attributes["survey_note"]["confidence"] = 2
    assert (
        profile.validate(building).issues[0].path
        == "objects['b'].attributes['survey_note']['confidence']"
    )
    building = Object(
        id="b",
        semantic_type="https://github.com/dtcc-platform/dtcc-core/schemas/model#QualifiedCode",
    ...
    schema["classes"]["IdentifiedNote"] = {
        "is_a": "SurveyNote",
        "attributes": {"note_id": {"identifier": True}},
    }
    path.write_text(yaml.safe_dump(schema))
