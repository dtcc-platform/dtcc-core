...
from dtcc_core.model import City, Building, Object, Mesh, Point, Field, SemanticRegion, exchange
...
def city():
    ...
    city.dataset_context = DatasetContext(
        identity=DatasetIdentity(name='standard-schema', title='Standard schema'), metadata=DatasetMetadata(),
...
def test_default_city_io_version_and_explicit_bypass(city, tmp_path):
    ...
    assert (pb.version, pb.schema_id, pb.schema_version) == (6, SCHEMA_ID, DEFAULT_VERSION)
    ...
    assert restored.buildings[0].lod2.x == 1  # Ordinary access permits unfinished edits.
    with pytest.raises(ValueError, match=r"objects\['building'\].attributes\['measured_height'\]"):
        restored.save(path)
    assert path.read_bytes() == before
    restored.save(path, validate_schema=False)
    with pytest.raises(ValueError, match='Schema validation failed'):
        io.load_city(path)
    assert io.load_city(path, validate_schema=False).buildings[0].attributes['measured_height'] == -1


def test_generic_data_preserves_unfamiliar_semantics_and_open_metadata(city, tmp_path):
    unknown = Object(id='bench', semantic_type='urn:external:ChargingBench',
                     attributes={'id': 'external-id', 'parent': 'source-metadata', 'chargingPower': -1},
    ...
    assert result.attributes == unknown.attributes and result.relations == unknown.relations
...
def test_mesh_wrapper_and_standalone_field_use_the_standard_schema(tmp_path):
    mesh = Mesh(vertices=np.array([[0., 0., 0.], [1., 0., 0.], [0., 1., 0.]]),
                faces=np.array([[0, 1, 2]]),
                regions=[SemanticRegion(NS + 'GroundSurface', np.array([0])),
    ...
    with pytest.raises(ValueError, match='out-of-range vertex index'):
        mesh.save(path, validate_schema=False)
    field = Field(name='temperature', unit='K', association='sample', values=np.array([280.]))
...
def test_bypass_never_accepts_malformed_or_unknown_wire_declarations():
    ...
    for name, value, message in [('version', 99, 'Unsupported model'),
...
def test_schema_only_field_metadata_rule_uses_the_native_field(tmp_path):
    ...
    value = Field(name='temperature', unit='C', association='sample', values=np.array([20.]))
