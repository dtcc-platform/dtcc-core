...
from dtcc_core.model import (
    Bounds, Building, City, Field, Grid, Landuse, LanduseClasses, LineString,
    Mesh, MultiLineString, MultiSurface, Object, Point, PointCloud, Raster,
    RoadNetwork, SemanticRegion, SensorCollection, Solid, Surface, Terrain,
    Tree, VehicleCollection, VolumeGrid, VolumeMesh, exchange,
)
...
def add_pavilion(city, x, y):
    building = feature(city, 'synthetic-pavilion', cls=Building,
                       description='Demonstration pavilion with a sealed interior cavity',
                       storeys_above_ground=2, storeys_below_ground=0,
                       estimated_height=8., function=['education'], usage=['demonstration'],
                       height_measurements=[{'value': 8., 'unit': 'm',
                           'high_reference': 'top_of_roof', 'low_reference': 'ground_surface',
                           'status': 'estimated', 'source': SYNTHETIC}])
    attach(building, MultiSurface(surfaces=[rectangle(x, y, 0., 20., 14.)]),
           'footprint', lod='0')
    attach(building, box(x, y, 0., 20., 14., 8.), 'massing', lod='1')
    solid = box(x, y, 0., 20., 14., 8.)
    window = np.array([[x+3, y, 3.], [x+7, y, 3.], [x+7, y, 5.], [x+3, y, 5.]])
    door = np.array([[x+12, y, .2], [x+14, y, .2], [x+14, y, 2.8], [x+12, y, 2.8]])
    solid.surfaces[2].holes = [window[::-1].copy(), door[::-1].copy()]
    solid.surfaces.extend([Surface(vertices=window), Surface(vertices=door)])
    interior = box(x+1, y+1, 1., 18., 12., 6.)
    solid.surfaces.extend(Surface(vertices=s.vertices[::-1].copy()) for s in interior.surfaces)
    solid.shells = [np.arange(8, dtype=np.int64), np.arange(8, 14, dtype=np.int64)]
    solid.regions = [region('GroundSurface', [0]), region('RoofSurface', [1]),
                     ...
                     region('FloorSurface', [8]), region('CeilingSurface', [9]),
                     region('InteriorWallSurface', [10, 11, 12, 13])]
    solid.fields = [field('surface_temperature', np.linspace(288., 305., 14).astype('float32'),
                          'face', 'K')]
    attach(building, solid, 'detailed_shells', lod='3')
    panel = feature(city, 'synthetic-solar-panel', nominal_power_w=320.)
    panel.semantic_type = 'https://example.org/dtcc/flagship#SolarPanel'
    panel.attributes['semantic_note'] = 'Custom vocabulary; admitted as generic Object, not a declared standard schema class'
    panel.relations = {'mounted_on': [building.id]}
    surface = rectangle(x+2, y+2, 8.1, 3., 2.)
    surface.vertices[2:, 2] += .5
    surface.fields = [field('power', np.array([240.], dtype='float32'), 'geometry', 'W')]
...
def add_landscape(city, x, y):
    ...
    attach(land, MultiSurface(surfaces=[rectangle(x, y+30, .05, 100., 60.)]), 'landuse', lod='0')
    cover = feature(city, 'synthetic-meadow', 'PlantCover', average_height=.6)
    attach(cover, MultiSurface(surfaces=[rectangle(x+5, y+35, .1, 42., 45.)]), 'cover', lod='1')
    for i in range(12):
        tx, ty = x+8+(i % 6)*15, y+95+(i // 6)*16
        tree = feature(city, f'synthetic-tree-{i:02d}', cls=Tree,
                       species='Tilia cordata')
        tree.position = np.array([tx, ty, 0.])
        tree.height, tree.crown_radius = 8.+i % 3, 3.
        attach(tree, Point(x=tx, y=ty, z=0.), 'location', lod='0')
        crown = np.array([[tx, ty, tree.height], [tx+3, ty, tree.height-3],
                          [tx, ty+3, tree.height-3], [tx-3, ty, tree.height-3],
                          [tx, ty-3, tree.height-3], [tx, ty, tree.height-6]])
        faces = np.array([[0, 1, 2], [0, 2, 3], [0, 3, 4], [0, 4, 1],
                          [5, 2, 1], [5, 3, 2], [5, 4, 3], [5, 1, 4]])
        attach(tree, Mesh(vertices=crown, faces=faces), 'crown', role='procedural_crown')
    water = feature(city, 'synthetic-pond', 'WaterBody', function=['retention'])
    solid = box(x+55, y+40, -2., 35., 30., 2.)
    solid.regions = [region('WaterGroundSurface', [0]),
    ...
    shape = box(0., 0., 0., 2., .6, .45)
    shape.transform.set_translation(x+25, y+25, 0.)
    angle = np.pi / 6
    shape.transform.affine[:2, :2] = [[np.cos(angle), -np.sin(angle)],
                                    [np.sin(angle), np.cos(angle)]]
    attach(bench, shape, 'local_shape', lod='2')
    attach(bench, Point(x=x+25, y=y+25, z=0.), 'location', lod='0')
    bench.attributes['coordinate_note'] = 'local_shape uses its own local-to-CRS affine; location is in CRS coordinates'


def add_transport(city, x, y):
    road = feature(city, 'synthetic-road', 'Road', function=['local_access'])
    axis = LineString(vertices=np.array([[x-5, y-10, 0.], [x+50, y-10, 0.], [x+110, y-10, 0.]]))
    axis.fields = [field('traffic_flow', np.array([120., 80.], dtype='float32'), 'edge', 'vehicles/h')]
    attach(road, axis, 'centerline', lod='0', role='centerline')
    surface = MultiSurface(surfaces=[rectangle(x-5, y-14, .02, 115., 8.),
                                     rectangle(x-5, y-6, .03, 115., 2.)])
    surface.regions = [region('TrafficArea', [0], surface_material='asphalt'),
    ...
    attach(network, MultiLineString(linestrings=[LineString(vertices=axis.vertices[e].copy())
                                                for e in network.edges]), 'links', role='routing_links')
    for id, kind, yy in [('rail', 'Railway', y-30), ('canal', 'Waterway', y+130)]:
        obj = feature(city, 'synthetic-'+id, kind)
        attach(obj, MultiLineString(linestrings=[LineString(vertices=np.array(
            [[x-5, yy, 0.], [x+110, yy, 0.]]))]), 'axis', lod='0')
    square = feature(city, 'synthetic-square', 'TransportSquare', function=['pedestrian'])
    attach(square, MultiSurface(surfaces=[rectangle(x+40, y, .02, 40., 22.)]), 'surface', lod='1')
    vehicles = feature(city, 'synthetic-vehicles', cls=VehicleCollection,
                       timestamp='2026-09-13T12:00:00Z')
    for i in range(3):
        vehicle = feature(vehicles, f'synthetic-vehicle-{i}',
                          vehicle_type='bus', timestamp='2026-09-13T12:00:00Z')
        point = Point(x=x+i*25, y=y-10, z=1.)
        point.fields = [field('velocity', np.array([[6.+i, 0., 0.]], dtype='float32'), 'sample', 'm/s', 3)]
...
def add_numerics(city, x, y, bounds):
    ...
    xx = np.arange(bounds.xmin-10, bounds.xmax+20, 10.)
    yy = np.arange(bounds.ymin-10, bounds.ymax+20, 10.)
    X, Y = np.meshgrid(xx, yy)
    Z = -2.5 + .3*np.sin((X-x)/80)*np.cos((Y-y)/90)
    ...
    mesh.fields = [field('ground_temperature', (290.+3*np.sin(X.ravel()/30)).astype('float32'), 'vertex', 'K')]
    ...
    cloud.fields = [field('confidence', np.full(len(vertices), .95, dtype='float32'), 'sample', '1')]
    attach(terrain, cloud, 'ground_samples', role='synthetic_samples')
    dem = builder.build_terrain_dem(
        cloud, 10., unit='m', vertical_reference=NAP,
    ...
    simulation = feature(city, 'synthetic-flow-domain',
                         formulas={'velocity': '[2 + 0.05*z, 0.2*sin(x/10), 0]',
                                   'pressure': '101325 - 12*z',
                                   'concentration': '20*exp(-((x-15)^2+(y-12)^2)/200)'},
                         timestamp='2026-09-13T12:00:00Z', solver=None)
    grid = Grid(width=8, height=6)
    grid.bounds = Bounds(xmin=x, ymin=y+145, xmax=x+40, ymax=y+175)
    grid.fields = [field('surface_runoff', np.linspace(0, 2, grid.num_cells).astype('float32'), 'cell', 'mm/h')]
    attach(simulation, grid, 'runoff_grid', role='analysis_grid')
    volume = VolumeGrid(width=6, height=4, depth=3)
    volume.bounds = Bounds(xmin=x, ymin=y+145, zmin=0., xmax=x+40, ymax=y+175, zmax=20.)
    volume.fields = [field('air_temperature', np.linspace(289, 296, volume.num_cells).astype('float32'), 'cell', 'K')]
    ...
    coordinates = np.array([[i*5., j*5., k*5.] for k in range(5) for j in range(7) for i in range(9)])
    cells = []
    ...
    for k in range(4):
        for j in range(6):
            for i in range(8):
                a = k*63+j*9+i
                corners = np.array([a, a+1, a+9, a+10, a+63, a+64, a+72, a+73])
                cells.extend(corners[pattern])
    cells = np.asarray(cells, dtype=np.int32)
    v = coordinates[cells]
    assert np.all(np.linalg.det(v[:, 1:]-v[:, :1]) > 0)
    xyz = coordinates + [x, y+145, 0.]
    tetra = VolumeMesh(vertices=xyz, cells=cells, markers=np.arange(len(cells), dtype='int32') % 3)
    wind = np.column_stack([2.+.05*coordinates[:, 2], .2*np.sin(coordinates[:, 0]/10), np.zeros(len(xyz))])
    centres = coordinates[cells].mean(axis=1)
    tetra.fields = [field('velocity', wind.astype('float32'), 'vertex', 'm/s', 3),
                     field('pressure', 101325.-12*coordinates[:, 2], 'vertex', 'Pa'),
                     field('tracer_concentration', (20*np.exp(-((centres[:, 0]-15)**2+
                           (centres[:, 1]-12)**2)/200)).astype('float32'), 'cell', 'ug/m3')]
    attach(simulation, tetra, 'tetrahedra', role='simulation_volume')
    stations = feature(city, 'synthetic-sensors', cls=SensorCollection,
                       timestamp='2026-09-13T12:00:00Z')
    for i in range(4):
        sensor = feature(stations, f'synthetic-sensor-{i}',
                         phenomenon='air_temperature', timestamp='2026-09-13T12:00:00Z')
        p = Point(x=x+5+i*9, y=y+150, z=2.)
        p.fields = [field('air_temperature', np.array([291.+i], dtype='float32'), 'sample', 'K')]
...
def enrich(city, bounds, *, mesh_real=True):
    ...
    all_bounds = Bounds(xmin=x-15, ymin=min(y-35, bounds.ymin),
                        xmax=bounds.xmax, ymax=max(y+180, bounds.ymax))
    ...
    if mesh_real:
        ...
        mesh.fields = [Field(name='triangle_area', values=area, unit='m2', association='face',
                       ...
                       field('solar_irradiance', (250+650*np.maximum(mesh.normals[:, 2], 0)).astype('float32'), 'face', 'W/m2')]
        attach(part, mesh, 'flagship_boundary_mesh', lod='2.2', role='derived_boundary_mesh')
        city.attributes['derived_representations'] = [{
            'object_id': part.id, 'representation_id': 'flagship_boundary_mesh',
            'source_lod': '2.2', 'method': 'dtcc_mesher boundary triangulation',
            'synthetic_fields': ['solar_irradiance'], 'derived_fields': ['triangle_area']}]
    city.attributes['synthetic_area_origin'] = [x, y, 0.]
    return city
...
def source_subset(document, selected):
    ...
    result['CityObjects'] = {id: obj for id, obj in result['CityObjects'].items() if id in selected}
...
def inventory(city):
            ...
            for geom in geometries(rep.geometry):
                ...
                for f in getattr(geom, 'fields', []):
                    fields.append({'object_id': obj.id, 'representation_id': rep_id,
                                   ...
                                   'shape': list(f.values.shape), 'dtype': str(f.values.dtype)})
    return {'objects': len(object_list), 'native_object_types': dict(sorted(native.items())),
            ...
            'fields': fields, 'relation_targets': sum(len(ids) for o in object_list for ids in o.relations.values())}
...
def preview(city, path):
    ...
    ax = fig.add_axes([.01, .12, .73, .78], projection='3d', facecolor='#101e2b')
    detail = fig.add_axes([.73, .43, .26, .37], projection='3d', facecolor='#101e2b')
    ...
    for obj in objects(city):
        ...
        selected = selected or next((r for r in records if r.lod == '2.2' and isinstance(r.geometry, Solid)), None)
        selected = selected or next((r for r in records if isinstance(r.geometry, (Solid, MultiSurface, Surface, Mesh))), None)
        if selected is None:
            continue
        g = selected.geometry
        synthetic = obj.attributes.get('synthetic', False)
        color = '#d5bb82' if not synthetic else '#53c7b5'
        if isinstance(obj, Tree): color = '#539864'
        if isinstance(obj, Terrain): color = '#304b48'
        if obj.semantic_type == NS+'WaterBody': color = '#549fe2'
        if obj.id == 'synthetic-meadow': color = '#567c48'
        ...
        polygons = [(p @ g.transform.affine[:3, :3].T + g.transform.affine[:3, 3])-origin for p in polygons]
        seen.extend(p for p in polygons)
        for p in polygons:
            local = p-p[0]
            normal = np.cross(local, np.roll(local, -1, axis=0)).sum(axis=0)
            length = np.linalg.norm(normal)
            light = abs(float(normal @ np.array([.3, -.4, .866])))/length if length else 0.
            colors.append(np.array(to_rgb(color))*(.65+.35*light))
        if obj.id == 'synthetic-pavilion':
            # Triangulate the exterior only so front-wall holes display correctly.
            exterior = MultiSurface(surfaces=copy.deepcopy(g.surfaces[:8]),
            ...
            for r in triangulated.regions:
                if r.semantic_type == NS+'Window': facecolors[r.indices] = to_rgb('#52a3e2')
                if r.semantic_type == NS+'Door': facecolors[r.indices] = to_rgb('#e5a968')
            detail.add_collection3d(Poly3DCollection(triangles, facecolors=facecolors,
                                                    edgecolors='none'))
    ...
    ax.add_collection3d(Poly3DCollection(seen, facecolors=colors, edgecolors='#172f39', linewidths=.12))
    ...
    fig.text(.05, .93, 'DTCC  /  DELFT FLAGSHIP', color='white', fontsize=25, weight='bold')
    fig.text(.05, .885, 'Real city geometry + a synthetic development laboratory', color='#a9c0cc', fontsize=13)
    fig.text(.76, .80, 'SYNTHETIC PAVILION', color='white', fontsize=11, weight='bold')
    fig.text(.76, .37, 'LoD 0 / 1 / 3\nWindow and door regions\nTwo shells, interior surfaces\nFace-associated temperature',
             color='#a9c0cc', fontsize=11, linespacing=1.6)
    fig.text(.05, .115, 'SAND  source buildings     TEAL  synthetic features     BLUE  synthetic water', color='#c7d8dd', fontsize=11)
    fig.text(.05, .078, 'Native file also contains rasters, point samples, grids, tetrahedra, fields, sensors and relations.', color='#a9c0cc', fontsize=10)
    fig.text(.05, .045, 'Overview: heights ×2; one representation per object; polygon holes/interior shells simplified. Pavilion detail: true proportions.', color='#90aab7', fontsize=9)
    fig.text(.99, .012, CREDIT+'  |  CC BY 4.0  |  https://docs.3dbag.nl/en/copyright/',
             color='#a9c0cc', fontsize=8, ha='right')
...
def main():
    ...
    centre = np.array([(anchor_bounds.xmin+anchor_bounds.xmax)/2, (anchor_bounds.ymin+anchor_bounds.ymax)/2])

    def distance(b):
        v = np.array([(b.bounds.xmin+b.bounds.xmax)/2, (b.bounds.ymin+b.bounds.ymax)/2])
    ...
    (args.output/'source-neighbourhood.city.json').write_text(json.dumps(subset, separators=(',', ':'))+'\n')
    ...
    city.attributes['flagship'] = {'version': '1.0.0', 'name': 'Delft flagship development model',
        'source_url': SOURCE_URL, 'source_sha256': SOURCE_SHA256, 'source_release': None,
    ...
    city.dataset_context = DatasetContext(
        identity={'name': 'dtcc-flagship-delft-v1', 'title': 'Delft flagship development model'},
        metadata={'crs': [CRS]}, presentation={},
        request={'dataset_name': 'dtcc-flagship-delft-v1'},
        provenance={'sources': [{'url': SOURCE_URL, 'sha256': SOURCE_SHA256, 'credit': CREDIT,
                                  'license': 'https://creativecommons.org/licenses/by/4.0/'},
                                 {'description': SYNTHETIC}],
                    'processing_steps': [{'operation': 'generate_flagship_model.py', 'version': '1.0.0',
